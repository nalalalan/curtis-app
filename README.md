...
- Note matching separates score-derived notes from reference-audio traces. Rhythm and octave are ignored for loose reference matching, but Curtis does not call that score detection. A scanned PDF plus guessed `scorePitchClassSequences` is ignored unless the sequence has an explicit symbolic-score source, manual source confirmation, MusicXML/source-confirmed status, or exact score-location metadata. The backend can parse inline, local `.musicxml`, or local `.mxl` symbolic scores, extract the solo-violin note sequence, keep written flats such as B-flat on the correct staff position while matching their pitch class, and create pending phrase candidates when the played pitch-class sequence meets the target-specific source-score run gate. The Scherzo-Tarantelle five-note actual-source crop `Bb4 D5 C5 Bb4 D5` at `mm. 2-4` is rejected after visual review because the displayed score crop and note boxes did not agree closely enough with the transcription. Reference-audio phrase candidates are collected across the full detected-series set and ranked by exact score status, score-derived status, distinct pitch-class content, and length, so longer useful candidates are no longer hidden behind early repeated-pitch runs. The tracker exposes pending phrase candidates as source-score verification targets, while these candidates remain pending evidence and are not accepted as score evidence. Single-pitch and multi-note score crops are withheld unless the visible score noteheads, register, spelling, exact range, note box centers, audio transcription, score transcription, and truth-item acceptance all agree; one-note audio overlaps are not accepted as score evidence. Accepted score-truth items reject same-pitch-class octave mismatches such as A4 versus A5 and reject score notes without octave/register. Detected D4/A4 audio fragments remain audio-checked transcription evidence only unless they match accepted symbolic score notes. Broader sheet-music snippets stay hidden until the played notes have exact score-location agreement; proportional score-region estimates are not emitted or rendered as score evidence. Violin-playing evidence without a confirmed score is treated as possible repertoire or a score-free technique exercise rather than as a failed score match.
- Long-phrase source truth is now a separate exact-MIDI manifest. The current Wieniawski source-positive phrase verifies `G5 F5 A5 G#5 F5` against the parsed MusicXML excerpt, but it is source-only until paired audio also passes the audio-agreement gate. Known false phrase maps, including the old `D C Bb` source map, the rejected `Bb4 D5 C5 Bb4 D5` lane, and the current live queued `G5 G5 A#5 A#5 A#5 A#5 A5` target, are blocked as regression cases.
