...
- Note matching separates score-derived notes from reference-audio traces. Rhythm and octave are ignored for loose reference matching, but Curtis does not call that score detection. A scanned PDF plus guessed `scorePitchClassSequences` is now ignored unless the sequence has an explicit symbolic-score source, manual source confirmation, MusicXML/source-confirmed status, or exact score-location metadata. Source-confirmed single-pitch score facts, such as Alan-confirmed A in Scherzo-Tarantelle, can create a pitch-anchor group that shows a score-side A staff note beside the played audio/video/transcription pair while exact measure and score crop remain pending. Scherzo-Tarantelle currently has a local reference-audio pitch-class trace, not a measure-by-measure symbolic score parse, so detected played sequences can produce grouped transcription, audio, and video evidence while `scoreLinked` remains false. Actual sheet-music snippets stay hidden until the played notes have exact score-location agreement; proportional score-region estimates are not emitted or rendered as score evidence. Violin-playing evidence without a confirmed score is treated as possible repertoire or a score-free technique exercise rather than as a failed score match.
