...
- Note matching is pitch-class sequence matching against a score or reference trace: rhythm and octave are ignored. Scherzo-Tarantelle currently has a local reference-audio pitch-class trace, not a measure-by-measure symbolic score parse, so detected played sequences can produce grouped transcription, audio, and video evidence but cannot produce actual sheet-music snippets. Actual sheet-music snippets stay hidden until the played notes have exact score-location agreement; proportional score-region estimates are not emitted or rendered as score evidence. Violin-playing evidence without a confirmed score is treated as possible repertoire or a score-free technique exercise rather than as a failed score match.
