...
- `/api/curtis/ops-check` exposes `review.dailyRecords`: analyzed practice-day records grouped from title-confirmed practice videos, with uploaded duration separated from real active violin-playing time, active-playing status, machine notation events, repeat groups, clips, heat-map layers, confirmed or uncertain pieces, and specific evidence-backed observations.
