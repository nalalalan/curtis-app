...
\metric{Active practice time}{27 min 45 s counted only from violin-positive media windows; uploaded duration is reported separately}
\metric{Unmeasured archive duration}{200 h 38 min still awaiting full media segmentation}
...
\metric{Analyzed practice days}{37 indexed days; 5 active-time measured daily records; 5 audio-evidence records; 4 score-audio-only records; 1 matched transcription record; 3 confirmed source labels with score targets and 0 exact score-matched snippets}
\metric{Machine pitch events}{Detected-active-section onset-aware pYIN plus onset-bounded spectral pitch rescue for fast passages, audio-agreement counts, pitch-sanity filtering, repeated-pitch-collapse guarding, active-coverage fields, title-labeled calibration handling, and fingerprints stored only when quality gates pass; broad candidate micro-transcription is withheld, while a strict note-sequence lane can display a fragment only when independent detectors agree on the clipped audio}
