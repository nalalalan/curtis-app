...
The production state uses the live backend at \url{https://curtis.aolabs.io/api/curtis/media-status}. The checked source state at 2026-05-13 04:05 PM EDT reported the values in Table~\ref{tab:state}. These are operating facts, not outcome claims.
...
\caption{Curtis source state checked at 2026-05-13 04:05 PM EDT.}
\label{tab:state}
...
\metric{Estimated total practice time}{208 h 49 min extrapolated from the checked-window active-playing ratio; provisional because the checked windows remain sparse and sample-biased}
\metric{Measured practice time}{1 h 58 min 54 s of detected violin-playing footage; this is independent from transcription}
\metric{Video checked for practice time}{2 h 1 min 30 s of the 213 h 23 min uploaded-video ledger}
\metric{Active-practice scan}{215 active sub-intervals persisted from stored media samples; 42 min 9 s of scan-derived active audio; 82 sample results; first \texttt{violin 1} window checked as non-playing; 250 queued windows still pending owner media}
\metric{Unchecked archive duration}{211 h 23 min still awaiting violin-playing detection}
\metric{Media samples}{82 active-scan sample results; 72 active violin; 10 checked non-violin}
...
\metric{Analyzed practice days}{39 indexed days; 3 practice-time measured daily records; 3 audio-evidence records; 2 score-audio-only records; 1 detected-note-fragment record; 3 confirmed source labels with score targets and 0 exact score-matched snippets}
...
\metric{Note-match windows}{14 pitch-sequence note-match groups; 0 exact score-aligned windows}
