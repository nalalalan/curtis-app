...
The production state uses the live backend at \url{https://curtis.aolabs.io/api/curtis/media-status}. The checked source state at 2026-05-14 07:04 PM EDT reported the values in Table~\ref{tab:state}. These are operating facts, not outcome claims.

A later implementation check at 2026-05-14 08:29 AM EDT corrected and extended the Scherzo-Tarantelle score-truth source. The previous \texttt{D A\# G D} source-score phrase is rejected as an accepted symbolic-score match because the local IMSLP solo-violin score shows the opening source phrase as \texttt{D C Bb D C Bb D}. The accepted 2026-05-03 Scherzo-Tarantelle phrase now reports detected notes \texttt{A\#4 D5 C5 A\#4 D5}; for accepted display it is score-spelled as \texttt{Bb4 D5 C5 Bb4 D5} against the same source score notes. The 2026-05-14 07:04 PM EDT actual-source score-gate update adds an exact-range source crop for that five-note subrange and prohibits generated notation SVGs from appearing as accepted score snippets. Generated notation remains available only as the transcription-side rendering; the score panel must use a non-generated source-score image with the same parsed score-note range. The 2026-05-14 09:45 PM EDT spelling-context update closes a separate visual mismatch: score-matched transcription rendering now carries the source key signature into the transcription panel, and accepted score panels require explicit visual-range agreement and score-spelling agreement in addition to actual-source-image agreement. This prevents a source score written as \texttt{Bb} in G minor from being displayed as an unqualified natural-position note on the transcription side. A later 2026-05-14 01:50 PM EDT deployment promoted accepted score phrases into score-coordinate heat-map fragments, so the same verified phrase now feeds the daily heat map and main blocker text instead of remaining only a match card. The 2026-05-14 02:51 PM EDT target-triage update checks four-note-or-longer reference-audio targets against the current verified symbolic score excerpt before any promotion attempt. In the current live state, the top source target is \texttt{D D D\# D D A\# G}; the verified score map is \texttt{D C A\# D C A\# D}; the best contiguous overlap is \texttt{D}, or 1/7 notes, so the target is classified as \texttt{source\_score\_sequence\_not\_found}, not accepted score evidence. The 2026-05-14 03:49 PM EDT score-map workbench update added a local source-map verification queue with 779 unaccepted score glyph candidates across 7 staves from page 2 of the local IMSLP solo part. The 2026-05-14 04:24 PM EDT note-hypothesis update filters that queue into 172 unaccepted likely notehead hypotheses with staff-position pitch labels. The 2026-05-14 05:17 PM EDT review-packet update fixed the skipped-staff failure mode in the source detector, expands page-2 coverage to 951 unaccepted score glyphs and 214 unaccepted note hypotheses across 8 staves, and writes 8 staff-level source review packets. The 2026-05-14 06:15 PM EDT score/transcription visual-agreement update requires detected MIDI notes, score MIDI notes, and the displayed score image source to agree before any score panel renders; the 07:04 PM update makes that displayed score source an actual score crop, not a generated facsimile; the 09:45 PM update requires the exact visible source range and notation spelling context to agree before that crop can count as accepted score evidence. The tracker is expected to report 65.628\% exact weighted progress after deployment, with 1 accepted long phrase, 1 accepted measure match, 1 exact score-aligned window, 1 score-coordinate heat-map fragment, 1 score-visual lock, 1 actual-source score lock, 12 pitch-sequence groups, 4 source-verification targets, 4 source-target score checks, 0 verified source targets, 7 verified symbolic score notes, 2 verified source snippets, 951 unaccepted score glyph candidates, 214 unaccepted score-note hypotheses, and 8 staff review packets. This is implementation progress only; a five-note phrase, rejected source-target checks, and unaccepted score-glyph, note-hypothesis, or review-packet candidates do not solve full long-phrase or full-session transcription.
...
\caption{Curtis source state checked at 2026-05-14 07:04 PM EDT.}
\label{tab:state}
...
\metric{Estimated total practice time}{220 h 51 min extrapolated from the checked-window active-playing ratio; provisional because the checked windows remain sparse and sample-biased}
...
\metric{Video checked for practice time}{2 h 30 min of the 234 h 57 min uploaded-video ledger}
...
\metric{Unchecked archive duration}{232 h 27 min still awaiting violin-playing detection}
...
\metric{Note-match windows}{First accepted source-backed five-note Scherzo-Tarantelle phrase on 2026-05-03; detected notes \texttt{A\#4 D5 C5 A\#4 D5} are accepted only because their MIDI values match source score notes \texttt{Bb4 D5 C5 Bb4 D5}; the accepted score panel now uses an actual source-score crop for that same subrange, while generated notation is confined to the transcription side; longer phrase and rhythm transcription remain unsolved}
\metric{Transcription solve tracker}{65.628\% expected exact weighted progress after actual-source score-gate update; 1 accepted long phrase; 1 accepted measure match; 1 exact score-aligned window; 1 score-coordinate heat-map fragment; 1 score-visual lock; 1 actual-source score lock; 12 pitch-sequence groups; 4 source-verification targets; 4 source-target score checks; 0 verified source targets; 7 verified symbolic score notes; 2 verified source snippets; 951 unaccepted score glyph candidates; 214 unaccepted score-note hypotheses; 8 staff review packets}
