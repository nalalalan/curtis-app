...
\metric{Transcription solve tracker}{Current live state after the Staff 4 anchored-expansion harness and raw-series expansion search: 1 accepted exact-MIDI source/audio phrase counter item, still only a five-note measure-window rather than a solved long phrase; 1 accepted source/audio measure-window lane in the truth manifest; 1 accepted verified source snippet; 3 anchored Staff 4 expansion checks and 0 accepted expansions; the best expansion keeps a five-note exact prefix but blocks at \texttt{Eb5} versus \texttt{D5}; expansion search now includes raw detected note series as well as ranked candidate cards; a Staff 4 phrase-audit packet generator can extract the exact blocking audio/video window, rerun pYIN/YIN/spectral checks, and store pitch-trace and spectrogram artifacts without accepting the failed expansion; 3 source-positive exact-MIDI phrases are verified against MusicXML; 3 known false phrase regressions are blocked; gold review candidates can now be accepted or rejected before training; remaining score-glyph review packets stay implementation support, not accepted evidence}
