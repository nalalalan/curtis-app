...
A later deployment check at 2026-05-14 07:52 AM EDT corrected the Scherzo-Tarantelle score-truth source and verified the ranked phrase-candidate path against the deployed site. The previous \texttt{D A\# G D} source-score phrase is rejected as an accepted symbolic-score match because the local IMSLP solo-violin score shows the opening source phrase as \texttt{D C Bb D C Bb}. The accepted 2026-05-03 Scherzo-Tarantelle groups now report detected pitch classes \texttt{A\# D C A\#} against symbolic score pitch classes \texttt{A\# D C A\#}; the source score notes are written \texttt{Bb4}, \texttt{D5}, \texttt{C5}, and \texttt{Bb4}. The displayed score panel uses the actual cropped IMSLP source-score image for the verified phrase, while the generated notation image is retained only as a fallback. The live transcription tracker now reports 48.642 percent exact progress, two accepted measure matches, two exact score-aligned windows, twelve pitch-sequence groups, one long phrase candidate pending source-score verification, three score targets, one local score PDF, six unique symbolic score notes, one verified source snippet, one exact score location category, and zero accepted long phrases. The highest-ranked pending phrase candidate remains \texttt{D D D\# D D A\# G}; the site exposes that sequence directly as the pending phrase target. This is implementation progress only; the phrase candidate is not accepted score evidence until its score-side notes are source-verified.
...
\metric{Analyzed practice days}{39 indexed days; 3 practice-time measured daily records; 3 audio-evidence records; 2 score-audio-only records; 1 detected-note-fragment record; 3 confirmed source labels, score targets, and a source-backed Wieniawski symbolic opening motif}
...
\metric{Note-match windows}{2 deployed source-backed symbolic Scherzo-Tarantelle measure matches on 2026-05-03; detected pitch classes \texttt{A\# D C A\#} match source score pitch classes \texttt{A\# D C A\#}; the previous \texttt{D A\# G D} source-score match is rejected; 1 ranked phrase candidate \texttt{D D D\# D D A\# G} remains pending source-score verification; accepted long phrases remain 0}
\metric{Transcription solve tracker}{48.642\% exact weighted progress; 2 accepted measure matches; 2 exact score-aligned windows; 12 pitch-sequence groups; 1 phrase candidate exposed as \texttt{D D D\# D D A\# G}; 3 score targets; 1 local score PDF; 6 unique symbolic score notes; 1 verified source snippet; 0 accepted long phrases}
