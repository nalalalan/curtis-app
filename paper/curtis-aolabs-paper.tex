...
Unverified machine note output now renders as draft notation paired to the exact audio window, not as final score-verified transcription. Each daily record can expose the paired audio/video evidence window from the same media sample, the draft staff generated from detected pitch events, and the current score-alignment limit. A spectral-onset rescue path now estimates pitch inside short onset-bounded violin segments when the pYIN path collapses, while the repeated-note trace guard prevents fast or complex playing from becoming a dominant repeated pitch, records the quality mode, and keeps unusable traces out of training data. Full-session score-linked transcription remains incomplete until every active playing section in the long-form practice videos is extracted and aligned to the score.
