...
\metric{Transcription solve tracker}{Current code state after the Staff 4 source-lane extension: 0 accepted live long phrases; 1 accepted source/audio measure-window lane in the truth manifest; 1 accepted verified source snippet; 1 additional source-only Staff 4 extension blocked from display by audio mismatch; 3 source-positive exact-MIDI phrases verified against MusicXML; 1 live-accepted exact-MIDI source/audio phrase; 3 known false phrase regressions blocked; gold review candidates can now be accepted or rejected before training; remaining score-glyph review packets stay implementation support, not accepted evidence}
