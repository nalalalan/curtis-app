...
The production state uses the live backend at \url{https://curtis.aolabs.io/api/curtis/media-status}. The checked source state at 2026-05-13 06:01 PM EDT reported the values in Table~\ref{tab:state}. These are operating facts, not outcome claims.
...
\caption{Curtis source state checked at 2026-05-13 06:01 PM EDT.}
\label{tab:state}
...
\metric{Estimated total practice time}{200 h 32 min extrapolated from the checked-window active-playing ratio; provisional because the checked windows remain sparse and sample-biased}
\metric{Measured practice time}{1 h 57 min of detected violin-playing footage; this is independent from transcription and cannot exceed checked video coverage}
\metric{Video checked for practice time}{2 h 4 min 30 s of the 213 h 23 min uploaded-video ledger}
\metric{Active-practice scan}{242 active sub-intervals persisted from stored media samples; 45 min 6 s of scan-derived active audio; 88 sample results; \texttt{violin 1} windows through \texttt{*540-630} checked; 250 queued windows still pending owner media}
\metric{Unchecked archive duration}{211 h 19 min still awaiting violin-playing detection}
\metric{Media samples}{88 active-scan sample results; 78 active violin; 10 checked non-violin}
