...
The production state uses the live backend at \url{https://curtis.aolabs.io/api/curtis/media-status}. The checked source state at 2026-05-13 11:20 AM EDT reported the values in Table~\ref{tab:state}. These are operating facts, not outcome claims.
...
\caption{Curtis source state checked at 2026-05-13 11:20 AM EDT.}
\label{tab:state}
...
\metric{Inventory}{216 indexed videos}
\metric{Practice candidates}{109 videos}
\metric{Long-form candidates}{89 videos}
\metric{Uploaded-video marker}{\texttt{violin 1}, uploaded 20 December 2025}
\metric{Uploaded-video ledger}{45 videos, 205 h 24 min uploaded session duration}
\metric{Estimated total practice time}{59 h extrapolated from the checked-window active-playing ratio; approximate until the full archive scan completes}
\metric{Measured practice time}{29 min 18 s of detected violin-playing footage; this is independent from transcription}
\metric{Video checked for practice time}{1 h 42 min of the 205 h 24 min uploaded-video ledger}
\metric{Unchecked archive duration}{203 h 42 min still awaiting violin-playing detection}
...
\metric{Analyzed practice days}{38 indexed days; 3 practice-time measured daily records; 3 audio-evidence records; 2 score-audio-only records; 1 detected-note-fragment record; 3 confirmed source labels with score targets and 0 exact score-matched snippets}
