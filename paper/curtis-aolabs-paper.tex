...
A later deployment check at 2026-05-14 06:52 AM EDT verified the first accepted source-backed score/audio measure and the ranked phrase-candidate path. The accepted 2026-05-03 Scherzo-Tarantelle group reports detected pitch classes \texttt{D A\# G D} against symbolic score pitch classes \texttt{D A\# G D}; the score notes are \texttt{D5}, written \texttt{Bb4}, \texttt{G4}, and \texttt{D5}. The live transcription tracker now reports 47.642 percent exact progress, one accepted measure match, one exact score-aligned window, twelve pitch-sequence groups, one long phrase candidate pending source-score verification, three score targets, one local score PDF, eight symbolic score notes, one exact score location, and zero accepted long phrases. The highest-ranked pending phrase candidate is \texttt{D D D\# D D A\# G}. This is implementation progress only; the phrase candidate is not accepted score evidence until its score-side notes are source-verified.
...
\metric{Transcription solve tracker}{47.642\% exact weighted progress; 1 accepted measure match; 1 exact score-aligned window; 12 pitch-sequence groups; 1 phrase candidate; 3 score targets; 1 local score PDF; 8 symbolic score notes; 1 exact score location; 0 accepted long phrases}
