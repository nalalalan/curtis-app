...
The production state uses the live backend at \url{https://curtis.aolabs.io/api/curtis/media-status}. The checked source state at 2026-05-09 06:03 UTC reported the values in Table~\ref{tab:state}. These are operating facts, not outcome claims.

\begin{table}[htbp]
\centering
\footnotesize
\caption{Curtis source state checked at 2026-05-09 06:03 UTC.}
\label{tab:state}
...
\metric{Active practice time}{12 min 52 s counted only from violin-positive media windows; uploaded duration is reported separately}
\metric{Unmeasured archive duration}{179 h 35 min still awaiting full media segmentation}
\metric{Media samples}{66 indexed windows; 27 violin-positive; 37 withheld}
...
\metric{Analyzed practice days}{35 indexed days; 5 active-time measured daily records; 5 audio-evidence records; 0 matched transcription records; 3 withheld candidate micro-transcription windows; 3 confirmed source labels with score targets and 0 exact score-matched snippets}
\metric{Machine pitch events}{Detected-active-section onset-aware pYIN plus onset-bounded spectral pitch rescue for fast passages, audio-agreement counts, pitch-sanity filtering, repeated-pitch-collapse guarding, active-coverage fields, title-labeled calibration handling, and fingerprints stored only when quality gates pass; candidate micro-transcription is withheld until audible note/rhythm match review passes}
...
The current system cannot predict acceptance at Curtis. It also cannot replace a musician, teacher, jury, studio lesson, or audition process. It can be wrong when the source media are incomplete, when audio is unavailable, when video frames hide the physical action, or when model review fails. The micro-transcription gate is currently held closed: candidate note runs can be stored, but no displayed transcription should be treated as solved until its notes and rhythms audibly match the paired clip. The useful claim is therefore limited: Curtis Media Review turns practice media into a persistent, source-bounded record that can support iteration over time.
