...
\metric{Active practice time}{8 min 59 s counted only from violin-positive media windows; uploaded duration is reported separately}
\metric{Unmeasured archive duration}{179 h 39 min still awaiting full media segmentation}
\metric{Media samples}{54 indexed windows; 18 violin-positive; 34 withheld}
...
\metric{Machine pitch events}{Detected-active-section onset-aware pYIN plus onset-bounded spectral pitch rescue for fast passages, pitch-sanity filtering, repeated-pitch-collapse guarding, active-coverage fields, and fingerprints stored only when quality gates pass; unverified events remain paired audio evidence until score alignment is reliable}
...
Machine note output now has two upstream gates. First, the media window must be classified as violin-positive before it can appear as playable evidence or count toward active time. Second, the note stream must be verified against the audio before it can render as sheet-music transcription; repertoire passages then need score alignment, while score-free exercises need repeated-pattern grouping. Unverified room, setup, laptop, silence, or non-playing samples remain withheld, and unverified pitch traces remain hidden support data rather than visible notation. A spectral-onset rescue path estimates pitch inside short onset-bounded violin segments when the pYIN path collapses, while the repeated-note trace guard prevents fast or complex playing from becoming a dominant repeated pitch, records the quality mode, and keeps unusable traces out of training data. Full-session transcription remains incomplete until every active playing section in the long-form practice videos is extracted, verified as violin-positive, verified against the audio, and aligned either to a score section or to a repeated technique pattern.
