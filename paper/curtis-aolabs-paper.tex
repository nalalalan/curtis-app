...
\metric{Transcription solve tracker}{Current live state after the Staff 4 adjacent-mining pass: 1 accepted exact-MIDI source/audio phrase counter item, still only a five-note measure-window rather than a solved long phrase; 1 accepted source/audio measure-window lane in the truth manifest; 1 accepted verified source snippet; 3 anchored Staff 4 expansion checks and 0 accepted expansions; the audited right-2 expansion is locked as a rejected regression because its sixth audio note is \texttt{D5} against source \texttt{Eb5}; \texttt{currentBest} now tests the adjacent right-1 window, which also blocks at \texttt{Eb5} versus \texttt{D5}; expansion search includes raw detected note series as well as ranked candidate cards; a dedicated Staff 4 mining pass searches stored May 3 audio-note windows for exact MIDI \texttt{75 75 72 75 75 75} and \texttt{75 75 72 75 75 75 72}, exposing either an exact candidate or the nearest miss; a Staff 4 phrase-audit packet generator can extract the exact blocking audio/video window, rerun pYIN/YIN/spectral checks, and store pitch-trace and spectrogram artifacts without accepting a failed expansion; 3 source-positive exact-MIDI phrases are verified against MusicXML; 4 known false phrase regressions are blocked; gold review candidates can now be accepted or rejected before training; remaining score-glyph review packets stay implementation support, not accepted evidence}
