...
\metric{Transcription solve tracker}{Current live state after the Staff 4 anchored-expansion harness: 1 accepted exact-MIDI source/audio phrase counter item, still only a five-note measure-window rather than a solved long phrase; 1 accepted source/audio measure-window lane in the truth manifest; 1 accepted verified source snippet; 3 anchored Staff 4 expansion checks and 0 accepted expansions; the best expansion keeps a five-note exact prefix but blocks at \texttt{Eb5} versus \texttt{D5}; 3 source-positive exact-MIDI phrases verified against MusicXML; 3 known false phrase regressions blocked; gold review candidates can now be accepted or rejected before training; remaining score-glyph review packets stay implementation support, not accepted evidence}
