...
The production state uses the live backend at \url{https://curtis.aolabs.io/api/curtis/media-status}. The checked source state at 2026-05-09 06:15 UTC reported the values in Table~\ref{tab:state}. These are operating facts, not outcome claims.

\begin{table}[htbp]
\centering
\footnotesize
\caption{Curtis source state checked at 2026-05-09 06:15 UTC.}
\label{tab:state}
...
\metric{Active practice time}{4 min 8 s counted only from violin-positive media windows; uploaded duration is reported separately}
\metric{Unmeasured archive duration}{157 h 58 min still awaiting full media segmentation}
...
\metric{Analyzed practice days}{33 indexed days; 3 active-time measured daily records; 3 audio-evidence records; 0 matched transcription records; 2 withheld candidate micro-transcription windows; 3 confirmed source labels with score targets and 0 exact score-matched snippets}
