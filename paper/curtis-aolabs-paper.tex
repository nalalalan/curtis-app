...
The production state uses the live backend at \url{https://curtis.aolabs.io/api/curtis/media-status}. The checked source state at 2026-05-13 07:55 PM EDT reported the values in Table~\ref{tab:state}. These are operating facts, not outcome claims.
...
\caption{Curtis source state checked at 2026-05-13 07:55 PM EDT.}
\label{tab:state}
...
\metric{Estimated total practice time}{197 h 59 min extrapolated from the checked-window active-playing ratio; provisional because the checked windows remain sparse and sample-biased}
\metric{Measured practice time}{2 h 15 min of detected violin-playing footage; this is independent from transcription and cannot exceed checked video coverage}
\metric{Video checked for practice time}{2 h 25 min of the 213 h 23 min uploaded-video ledger}
\metric{Active-practice scan}{313 active sub-intervals persisted from stored media samples; 51 min 48 s of scan-derived active audio; 102 sample results; \texttt{violin 1} windows through \texttt{*1800-1890} checked; 250 queued windows still pending owner media}
\metric{Unchecked archive duration}{210 h 58 min still awaiting violin-playing detection}
\metric{Media samples}{102 active-scan sample results; 90 active violin; 12 checked non-violin}
