...
The production state uses the live backend at \url{https://curtis.aolabs.io/api/curtis/media-status}. The checked source state at 2026-05-12 18:09 UTC reported the values in Table~\ref{tab:state}. These are operating facts, not outcome claims.

\begin{table}[htbp]
\centering
\footnotesize
\caption{Curtis source state checked at 2026-05-12 18:09 UTC.}
\label{tab:state}
...
\metric{Estimated total practice time}{66 h 14 min extrapolated from the checked-window active-playing ratio; approximate until the full archive scan completes}
\metric{Measured practice time}{33 min 6 s of detected violin-playing footage; this is independent from transcription}
...
\metric{Media samples}{80 indexed windows; 60 violin-positive; 20 withheld}
...
\metric{Analyzed practice days}{37 indexed days; 5 practice-time measured daily records; 5 audio-evidence records; 4 score-audio-only records; 1 detected-note-fragment record; 3 confirmed source labels with score targets, 15 pitch-sequence note-match groups, and 0 exact score-matched snippets}
