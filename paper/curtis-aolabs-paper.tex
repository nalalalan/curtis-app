...
The production state uses the live backend at \url{https://curtis.aolabs.io/api/curtis/media-status}. The checked source state at 2026-05-13 12:59 PM EDT reported the values in Table~\ref{tab:state}. These are operating facts, not outcome claims.
...
\caption{Curtis source state checked at 2026-05-13 12:59 PM EDT.}
\label{tab:state}
...
\metric{Estimated total practice time}{208 h 46 min extrapolated from the checked-window active-playing ratio; provisional because the checked windows remain sparse and sample-biased}
\metric{Measured practice time}{1 h 57 min 24 s of detected violin-playing footage; this is independent from transcription}
\metric{Video checked for practice time}{2 h of the 213 h 23 min uploaded-video ledger}
\metric{Active-practice scan}{214 active sub-intervals persisted from stored media samples; 41 min 42 s of scan-derived active audio; 250 queued windows still pending owner media}
\metric{Unchecked archive duration}{211 h 23 min still awaiting violin-playing detection}
\metric{Media samples}{80 indexed windows; 68 violin-positive; 12 withheld}
