...
A later implementation check at 2026-05-14 08:29 AM EDT corrected and extended the Scherzo-Tarantelle score-truth source. The previous \texttt{D A\# G D} source-score phrase is rejected as an accepted symbolic-score match because the local IMSLP solo-violin score shows the opening source phrase as \texttt{D C Bb D C Bb D}. The accepted 2026-05-03 Scherzo-Tarantelle phrase now reports detected pitch classes \texttt{A\# D C A\# D} against symbolic score pitch classes \texttt{A\# D C A\# D}; the source score notes are written \texttt{Bb4}, \texttt{D5}, \texttt{C5}, \texttt{Bb4}, and \texttt{D5}. The displayed score panel uses the actual cropped IMSLP source-score image for the verified phrase, while generated notation is retained only as a fallback. The implementation tracker now counts this as the first accepted long-phrase gate pass because it has an exact source-score location, at least five contiguous source score notes, local playable audio/video, and is not a single-pitch anchor. The deployed tracker reports 55.642\% exact weighted progress, 1 accepted long phrase, 2 accepted measure matches, 2 exact score-aligned windows, 12 pitch-sequence groups, 7 unique symbolic score notes, and 1 verified source snippet. This is implementation progress only; a five-note phrase does not solve full long-phrase or full-session transcription.
...
\metric{Transcription solve tracker}{55.642\% exact weighted progress; 1 accepted long phrase; 2 accepted measure matches; 2 exact score-aligned windows; 12 pitch-sequence groups; 7 unique symbolic score notes; 1 verified source snippet}
