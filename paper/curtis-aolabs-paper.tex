...
\metric{Transcription solve tracker}{Current live state after the Staff 4 source-lane extension: 1 accepted exact-MIDI source/audio phrase counter item, still only a five-note measure-window rather than a solved long phrase; 1 accepted source/audio measure-window lane in the truth manifest; 1 accepted verified source snippet; 1 additional source-only Staff 4 extension blocked from display by audio mismatch; 3 source-positive exact-MIDI phrases verified against MusicXML; 3 known false phrase regressions blocked; gold review candidates can now be accepted or rejected before training; remaining score-glyph review packets stay implementation support, not accepted evidence}
