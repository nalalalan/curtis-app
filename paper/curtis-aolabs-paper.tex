...
The 2026-05-15 09:20 PM EDT live targeted-reprocess verification used that route on \texttt{Njh8\_zq9\_DM-9465}. The deployed API then reported 67.375\% exact weighted progress, 1 accepted long-phrase counter item, 1 accepted measure-match counter item, 1 exact-source visible score group for 2026-05-03, and 1380 hidden transition-trace candidates across the currently stored transcription results. The public interface shows exactly one accepted 2026-05-03 Scherzo-Tarantelle group: source score \(\leftrightarrow\) local video/audio clip \(\leftrightarrow\) transcription, with the same visible pitch sequence \texttt{Bb D C Bb D} and exact notes \texttt{Bb4 D5 C5 Bb4 D5}. This is a verified short phrase lane, not a complete long-phrase solve: the broader 5/3 detected stream still contains repeated B-flat/D material and cannot yet produce a trustworthy multi-measure transcription without extending the verified source score map and improving fast-passage pitch tracking.
...
\metric{Analyzed practice days}{41 indexed days; 3 practice-time measured daily records; 3 audio-evidence records; 2 score-audio-only records; 1 detected-note-fragment record; 3 confirmed source labels, score targets, and a source-backed Wieniawski seven-note symbolic opening phrase}
...
\metric{Note-match windows}{One accepted 2026-05-03 Scherzo-Tarantelle source-score phrase after strict-source correction: detected \texttt{A\#4 D5 C5 A\#4 D5}, displayed \texttt{Bb4 D5 C5 Bb4 D5}, and matched to the same score-side notes in a corrected actual-source crop; longer phrase and rhythm transcription remain unsolved}
\metric{Transcription solve tracker}{65.126\% live exact weighted progress after strict-source correction; 1 accepted five-note source-score phrase; 1 accepted measure-level match; 1 exact score-aligned window; 1 score-coordinate heat-map fragment; 1 score-visual lock; 1 actual-source score lock; 0 accepted truth items; 2 source-verification targets queued for truth review; 2 source-target score checks; 0 verified longer source targets; 7 verified symbolic score notes; 1 verified source snippet; 951 unaccepted score glyph candidates; 214 unaccepted score-note hypotheses; 8 staff review packets}
