...
\metric{Analyzed practice days}{35 indexed days; 7 processed daily records; 5 audio-evidence records; 5 failed transcription records; 0 verified transcription records; 3 confirmed source labels with score snippets}
\metric{Machine pitch events}{Detected-active-section onset-aware pYIN with pitch-sanity filtering, repeated-pitch-collapse rejection, active-coverage fields, and fingerprints stored only when quality gates pass; unverified events fail the transcription gate until score alignment is reliable}
\metric{Specific observations}{Withheld unless grounded in reliable score-linked evidence}
...
\metric{Daily evidence surface}{Local sample playback, audio evidence windows, score heat-map target, and explicit whole-session transcription limit}
...
The first live readout is experimental but usable. The strongest immediate value is that the system has already converted a large, timestamped practice archive into a queryable corpus with captured media samples, stored section reviews, and title-confirmed practice records. The home screen has three top-level surfaces: a paper card, analyzed practice days, and repertoire. The first evidence row now reports the YouTube practice archive first, then the active violin time that has actually been measured, then the archive duration still unmeasured; this prevents the 179 h 48 min public practice ledger from being visually collapsed into the much smaller processed-clip total. Each analyzed day groups same-day videos, preserves uploaded duration, shows active-playing status, embeds playable local media samples, keeps paired audio/video evidence windows, shows a score-image heat-map target when a score packet exists, and names confirmed or uncertain piece evidence without promoting guesses. The daily view states when transcription failed the quality gate; unverified machine pitch events are retained internally as failed evidence rather than rendered as false sheet music. Active scans with no reliable transcription remain active-time evidence rather than notation coverage. The repertoire surface is evidence-backed: an entry appears only when a daily record carries confirmed source evidence, and each entry keeps practice dates, clips, score targets, active time when measured, and current blocker limits. Generic etude/caprice evidence remains outside the piece-title field, and possible or uncorroborated repertoire labels remain checkable against the source window. These statements are not generic pedagogy; they are extracted from stored review findings, sample indexes, source corrections, public-domain score targets, failed machine pitch events, and the uploaded-video ledger.
...
Unverified machine note output now fails the transcription gate in the primary interface. Each daily record can still expose the paired audio/video evidence window from the same media sample, but the system does not render those pitch events as music until the result can be aligned to the score and formatted as credible notation. A repeated-note collapse gate rejects pitch traces that reduce fast or complex playing to a dominant repeated pitch, records the failure mode, and prevents the trace from becoming training data. Full-session score-linked transcription remains incomplete until every active playing section in the long-form practice videos is extracted and aligned to the score.
