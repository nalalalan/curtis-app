...
\caption{Curtis source state checked at 2026-05-14 06:15 PM EDT.}
\label{tab:state}
...
\metric{Note-match windows}{First accepted source-backed five-note Scherzo-Tarantelle phrase on 2026-05-03; detected notes \texttt{A\#4 D5 C5 A\#4 D5} are accepted only because their MIDI values match source score notes \texttt{Bb4 D5 C5 Bb4 D5}; the score panel now uses the exact symbolic score slice because the available IMSLP crop covers a broader span; longer phrase and rhythm transcription remain unsolved}
\metric{Transcription solve tracker}{64.878\% exact weighted progress after score/transcription visual-agreement update; 1 accepted long phrase; 1 accepted measure match; 1 exact score-aligned window; 1 score-coordinate heat-map fragment; 1 score-visual lock; 12 pitch-sequence groups; 4 source-verification targets; 4 source-target score checks; 0 verified source targets; 7 verified symbolic score notes; 1 verified source snippet; 951 unaccepted score glyph candidates; 214 unaccepted score-note hypotheses; 8 staff review packets}
