...
\metric{Media samples}{20 indexed windows; playback withheld until violin-positive}
...
Machine note output now has an upstream media gate. Draft notation may render only when it is paired to an explicitly violin-positive audio window; unverified room, setup, laptop, silence, or non-playing samples remain withheld rather than appearing as transcription. A spectral-onset rescue path now estimates pitch inside short onset-bounded violin segments when the pYIN path collapses, while the repeated-note trace guard prevents fast or complex playing from becoming a dominant repeated pitch, records the quality mode, and keeps unusable traces out of training data. Full-session score-linked transcription remains incomplete until every active playing section in the long-form practice videos is extracted, verified as violin-positive, and aligned to the score.
