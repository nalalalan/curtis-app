...
The 2026-05-15 11:32 PM EDT live verification reported the strict post-update state: 46.125\% exact weighted implementation progress, 0 accepted long phrases, 0 accepted measures, 0 exact score-aligned windows, 0 actual-source score snippets, and 0 score-coordinate heat-map fragments. The 2026-05-03 daily record had 0 visible \texttt{matchGroups}, 6 queued \texttt{candidateMatchGroups}, and no score or reference link. The current top queued target is \texttt{A D D F\# D}, which remains rejected as accepted score evidence because the verified symbolic source map \texttt{D C A\# D C A\# D} gives only a one-note contiguous overlap.

The 2026-05-16 12:20 AM EDT source-octave correction closes a score-map error that would have kept Curtis in the same failure loop. Re-rendering the local IMSLP solo part showed that the opening symbolic map had been stored one octave too low. The source MusicXML now stores the opening as \texttt{D6 C6 Bb5 D6 C6 Bb5 D6}; the previous \texttt{D5 C5 Bb4 D5 C5 Bb4 D5} map is explicitly rejected by regression test. This change does not create an accepted long phrase. It makes verified symbolic score mapping stricter: a displayed score phrase must match parsed source-score MIDI notes, not only letter names or pitch classes, before any audio-agreed transcription can be promoted.
...
\metric{Analyzed practice days}{41 indexed days; 3 practice-time measured daily records; 3 audio-evidence records; 2 score-audio-only records; 1 detected-note-fragment record; 3 confirmed source labels, score targets, and a source-backed Wieniawski seven-note symbolic opening phrase now corrected to \texttt{D6 C6 Bb5 D6 C6 Bb5 D6}}
