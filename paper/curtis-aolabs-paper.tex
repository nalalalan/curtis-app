...
The production state uses the live backend at \url{https://curtis.aolabs.io/api/curtis/media-status}. The checked source state at 2026-05-12 21:07 UTC reported the values in Table~\ref{tab:state}. These are operating facts, not outcome claims.
...
\caption{Curtis source state checked at 2026-05-12 21:07 UTC.}
\label{tab:state}
...
\metric{Uploaded-video ledger}{44 videos, 201 h 6 min uploaded session duration}
\metric{Estimated total practice time}{66 h 16 min extrapolated from the checked-window active-playing ratio; approximate until the full archive scan completes}
\metric{Measured practice time}{33 min 7 s of detected violin-playing footage; this is independent from transcription}
\metric{Video checked for practice time}{1 h 40 min of the 201 h 6 min uploaded-video ledger}
\metric{Unchecked archive duration}{199 h 25 min still awaiting violin-playing detection}
\metric{Media samples}{80 indexed windows; 67 violin-positive; 13 withheld}
...
\metric{Analyzed practice days}{37 indexed days; 4 practice-time measured daily records; 4 audio-evidence records; 3 score-audio-only records; 1 detected-note-fragment record; 3 confirmed source labels with score targets, 15 pitch-sequence note-match groups, and 0 exact score-matched snippets}
...
\metric{Note-match windows}{15 pitch-sequence note-match groups; 0 exact score-aligned windows}
