...
\metric{Inventory}{209 indexed videos}
\metric{Practice candidates}{104 videos}
\metric{Long-form candidates}{84 videos}
\metric{Uploaded-video marker}{\texttt{violin 1}, uploaded 20 December 2025}
\metric{Title-confirmed uploaded-video ledger}{40 videos, 158 h 2 min uploaded session duration}
\metric{Active practice time}{2 min 53 s counted only from violin-positive media windows; uploaded duration is reported separately}
\metric{Unmeasured archive duration}{158 h still awaiting full media segmentation}
\metric{Media samples}{70 indexed windows; 30 violin-positive; 38 withheld}
...
\metric{Analyzed practice days}{33 indexed days; 3 active-time measured daily records; 3 audio-evidence records; 2 score-audio-only records; 1 audio-matched transcription fragment; 3 confirmed source labels with score targets and 0 exact score-matched snippets}
\metric{Machine pitch events}{Detected-active-section onset-aware pYIN plus onset-bounded spectral pitch rescue for fast passages, audio-agreement counts, pitch-sanity filtering, repeated-pitch-collapse guarding, active-coverage fields, title-labeled calibration handling, and fingerprints stored only when quality gates pass; broad candidate micro-transcription is withheld, while a strict single-note lane can display a fragment only when independent detectors agree on the paired clipped audio}
