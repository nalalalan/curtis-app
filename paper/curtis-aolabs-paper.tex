...
\vspace{0.35em}
\noindent\textbf{High-level music training produces dense evidence: daily practice videos, repeated repertoire, changing technical constraints, and periodic performance goals. Most of that evidence is difficult to use because it is stored as unstructured media rather than as a score-linked longitudinal record. We report Curtis, a live AO Labs system that converts messy long-form violin practice videos into structured, evidence-linked practice records using active practice detection, playable audio/video evidence windows, score alignment targets, heat-map targets, repertoire tracking, and passage-level performance observation. The checked live state on 8 May 2026 used the YouTube channel \texttt{@nalalan} as the primary source, indexed 211 public videos, marked 106 as practice candidates, marked 86 as long-form candidates, stored 26 media-sample index entries, retained 80 reviewed sections, and recorded 80 Curtis-focused findings. The uploaded-video ledger begins at \texttt{violin 1}, uploaded on 20 December 2025, and currently counts 42 violin-numbered or date-titled practice logs totaling 179 h 48 min of public session duration; this duration is not counted as active practice time. The live daily-record state contains 35 indexed practice days, 0 verified transcription records, 4 audio-evidence records, 9 violin-positive sample windows, and 17 withheld non-violin windows. Active violin practice time is counted only from media windows that are explicitly violin-positive; the current measured active total is 4 min 32 s, and all unverified room, setup, laptop, silence, or non-playing samples are withheld rather than counted as practice evidence. The primary interface has three top-level surfaces: a paper card, analyzed practice days, and repertoire. The analyzed-days evidence row separates indexed YouTube practice-archive duration, measured active violin time, and still-unmeasured archive duration so the interface does not hide the multi-hour corpus or overstate active hours. Each daily record now withholds embedded playback, paired audio, and active-time credit until the source window is violin-positive, and withholds sheet-music notation until score verification. Draft notation does not render as transcription; repeated single-pitch traces such as a spurious \texttt{D D D D} stream remain out of final notation and matching data. Repertoire entries are promoted only from confirmed daily-record evidence and keep possible labels, user-rejected false labels, weak evidence, and readiness scores inferred only from piece identification separate from confirmed repertoire and judged playing quality. The system uses GPT-5.5 for vision and text review and separate audio models for identification and verification, while preserving explicit limits: Curtis admission cannot be predicted from current samples, weak audio/video evidence is marked unjudged, whole-session transcription is not yet complete, and the system reports practice signals rather than admissions probability. The contribution is an experimental method for making practice visible, searchable, measurable, and tied to actual musical evidence.}
...
\metric{Active practice time}{4 min 32 s counted only from violin-positive media windows; uploaded duration is reported separately}
\metric{Unmeasured archive duration}{179 h 43 min still awaiting full media segmentation}
\metric{Media samples}{26 indexed windows; 9 violin-positive; 17 withheld}
...
\metric{Analyzed practice days}{35 indexed days; 4 processed daily records; 4 audio-evidence records; 0 verified transcription records; 3 confirmed source labels with score snippets}
...
\metric{Specific observations}{Rendered only when grounded in reliable score-linked evidence}
...
\metric{Daily evidence surface}{Local sample playback only for violin-positive windows, score heat-map target, notation withheld until score verification, and explicit whole-session transcription limit}
