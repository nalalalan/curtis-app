...
\metric{Transcription solve tracker}{Current state after the Staff 4 tiled-rescan and anchor-reproduction pass: 1 accepted exact-MIDI source/audio phrase counter item, still only a five-note measure-window rather than a solved long phrase; the accepted Staff 4 anchor is recoverable from the truth manifest even when the daily display has zero visible accepted match groups, but the current-detector source-audio rescan now explicitly checks whether that anchor is reproduced before further expansion; the first deployed tiled check processed five overlapping tiles, four note runs, and 286 primary-plus-candidate note events, then found zero exact adjacent Staff 4 candidates after 1547 stored/rescanned windows; source-audio search now uses up to five overlapping tiles from the same \texttt{*8835-8925} source sample instead of one short rescan window; 1 accepted source/audio measure-window lane in the truth manifest; 1 accepted verified source snippet; anchored Staff 4 expansion checks remain blocked until adjacent audio agrees with source \texttt{Eb5}; the audited right-2 expansion is locked as a rejected regression because its sixth audio note is \texttt{D5} against source \texttt{Eb5}; expansion search includes raw detected note series, ranked candidate cards, truth-manifest anchors, and source-audio-rescanned runs; a dedicated Staff 4 mining pass searches stored and rescanned May 3 audio-note windows for exact MIDI \texttt{75 75 72 75 75 75} and \texttt{75 75 72 75 75 75 72}, exposing either an exact candidate or the nearest miss; a Staff 4 phrase-audit packet generator can extract the exact blocking audio/video window, rerun pYIN/YIN/spectral checks, and store pitch-trace and spectrogram artifacts without accepting a failed expansion; 3 source-positive exact-MIDI phrases are verified against MusicXML; 4 known false phrase regressions are blocked; gold review candidates can now be accepted or rejected before training; remaining score-glyph review packets stay implementation support, not accepted evidence}
