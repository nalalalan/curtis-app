...
The tracker was refreshed through the 2026-05-15 04:01 PM EDT practice-time API unification pass. The system has a real media ledger, a deployed active-practice scan route, a working owner-sync path for captured media, a visible 100-point transcription-completion tracker with precise weighted progress, stored user-correction benchmarks, a persistent truth workbench for accepted/rejected/pending audio-score-transcription items, a local source-backed Wieniawski score PDF, a seven-note source-backed Wieniawski symbolic score opening phrase, a source-verification target queue, source-target score triage, an unaccepted score-glyph verification queue, an unaccepted score-note hypothesis queue, staff-level source review packets, and a conservative evidence gate. The score-side failure mode is now handled by an exact source crop, not by relabeling. The only accepted Scherzo-Tarantelle source crop is the boxed actual-IMSLP five-note range \texttt{Bb4 D5 C5 Bb4 D5}, derived from the local page-2 scan at the same symbolic range \texttt{mm. 2--4}. The matcher requires MIDI-level audio/score equality, written score spelling, exact visible note sequence, exact visible note-plus-octave sequence, actual-source image display, and local media before it can count the group as score evidence. The old A4/A5 single-note anchors remain rejected, and a one-note pitch anchor still cannot render unless the score note and octave are explicitly verified. The current top longer source target \texttt{D D D\# D D A\# G} remains unaccepted because the verified symbolic score map contains only \texttt{D C A\# D C A\# D} and gives only a one-note contiguous overlap. This is not solved long-phrase transcription yet; it creates one exact five-note source-score lane, makes the daily-record API share the canonical active-practice coverage ledger, and keeps longer passage transcription, rhythm, repeated attempts, full-score mapping, and full archive practice-time coverage open.
...
Current accepted transcription & One exact five-note Scherzo-Tarantelle phrase path is test-covered: detected \texttt{A\#4 D5 C5 A\#4 D5} is score-spelled as \texttt{Bb4 D5 C5 Bb4 D5}. This is still not a full passage. \\
Current accepted score match & One actual-source crop is accepted for \texttt{Bb4 D5 C5 Bb4 D5} at \texttt{mm. 2--4} when paired with the matching local audio/video series. The current \texttt{D D D\# D D A\# G} longer source target is still rejected for now with only 1/7 contiguous source overlap; 951 local score glyph candidates, 214 staff-position notehead hypotheses, 8 staff review packets, and 4 source-verification targets are queued for truth review but are not accepted score evidence. \\
...
\paragraph{2026-05-14 06:15 PM EDT score/transcription visual-agreement update} The public 2026-05-03 match had a real failure: the score panel showed a broad opening source crop even though the accepted transcription phrase was the narrower five-note slice \texttt{Bb4 D5 C5 Bb4 D5}. Curtis now treats score display as a separate acceptance gate. A score panel can render an actual source crop only when the crop's reference start and end exactly match the accepted phrase; broad covering crops are withheld. If the symbolic score notes are verified but an exact-range source crop does not exist, the score panel renders a generated excerpt from the exact symbolic score slice instead. The symbolic matcher also now requires MIDI-level equality between detected notes and parsed score notes, not only pitch-class similarity, before it can create a score-visual lock. The live 2026-05-03 phrase stores detected notes as \texttt{A\#4 D5 C5 A\#4 D5} but displays the score-spelled accepted sequence \texttt{Bb4 D5 C5 Bb4 D5} on both the score and transcription sides. The deployed tracker now reports 64.878 percent exact progress, 1 accepted long phrase, 1 accepted measure match, 1 exact score-aligned window, 1 score-coordinate heat-map fragment, and 1 score-visual agreement lock. This makes wrong score panels harder to publish; it does not solve full long-phrase transcription.

\paragraph{2026-05-14 07:04 PM EDT actual-source score-gate update} The previous visual-agreement fix still left one unacceptable loophole: a generated notation SVG could appear in the accepted score panel even though Alan asked for the actual piece. Curtis now requires an accepted score panel to use a non-generated source-score crop for the same reference range. The five-note 2026-05-03 Scherzo-Tarantelle match now has an exact-range source crop for \texttt{Bb4 D5 C5 Bb4 D5}; generated notation remains only as the transcription rendering. Accepted measure and long-phrase counters now require this actual-source score snippet in addition to local media, exact score location, and note agreement. The tracker is expected to report 65.628 percent exact progress after deployment, 1 actual-source score lock, 1 accepted long phrase, 1 accepted measure match, 1 exact score-aligned window, 1 score-coordinate heat-map fragment, and 2 verified source snippets. This guarantees the score panel represents the actual piece only for the verified crop range; it does not yet guarantee long phrase transcription beyond the five-note fragment.
...
\paragraph{2026-05-14 10:52 PM EDT visual-crop demotion update} Alan's follow-up review showed that the notes visible in the 2026-05-03 source crop still did not match the transcription. Curtis now rejects the Scherzo-Tarantelle source crops as review material unless the visible noteheads, register, written spelling, and exact range are independently verified against the transcription. The five-note 5/3 phrase remains a symbolic phrase candidate, but it no longer creates an accepted score panel, accepted measure match, accepted long phrase, exact score-aligned window, score-coordinate heat-map fragment, score-visual lock, actual-source score lock, or verified source snippet. The tracker is expected to fall to 43.628 percent exact progress if the live practice-time and score-queue state is otherwise unchanged. This is a regression lock: wrong score evidence is removed rather than cosmetically relabeled.

\paragraph{2026-05-15 12:45 AM EDT truth-workbench update} Curtis now has a persistent truth-workbench API and compact implementation-status surface for accepted, pending, and rejected audio-score-transcription evidence. A truth item can store the source clip, practice day, piece, detected notes, accepted notes, score notes, score source, score location, score image, and gate state. Accepted truth items are rejected at write time if the accepted audio note sequence and score note sequence disagree. The backend and frontend score gates also now require explicit visible-note-sequence verification before a score panel can count as a score-visual lock, actual-source score lock, accepted measure match, or accepted long phrase. This prevents the exact failure Alan found: a visually wrong score crop cannot count just because the metadata says the pitch sequence should match. The expected implementation tracker moves to 45.628 percent if the live practice-time and score-queue state is otherwise unchanged, because the missing truth path and queued truth-review surface now exist. It still has 0 accepted long phrases, 0 accepted measure matches, 0 exact score-aligned windows, 0 score-coordinate heat-map fragments, 0 score-visual locks, 0 actual-source score locks, and 0 accepted truth clips.

\paragraph{2026-05-15 03:20 PM EDT exact-source phrase update} Curtis now accepts exactly one actual-source Scherzo-Tarantelle phrase crop: \texttt{Bb4 D5 C5 Bb4 D5} from the local IMSLP page-2 solo-violin scan, aligned to the symbolic score range \texttt{mm. 2--4}. The crop boxes only the five matched noteheads and stores the source page pixel coordinates and exact visible note-plus-octave sequence. The symbolic matcher now refuses source snippets unless the visible source notes, exact source notes with octave, score spelling, score range, detected MIDI sequence, and local media all agree. Local regression tests verify that detected \texttt{A\#4 D5 C5 A\#4 D5} is displayed as \texttt{Bb4 D5 C5 Bb4 D5}, uses the actual source crop, creates a score-coordinate heat-map fragment, and keeps the rejected A4/A5 one-note anchors blocked. This is a real short-phrase pass and a stricter score-source gate; it still does not solve 10--30 s long-phrase transcription or full-session coverage.

\paragraph{2026-05-15 03:40 PM EDT deployment verification} The deployed Curtis site serves cache version \texttt{20260515-exact-source-phrase} on deployment \texttt{4fb78ce1}. The live 2026-05-03 entry shows one accepted score group: detected notes A\#4 D5 C5 A\#4 D5, score-spelled transcription Bb4 D5 C5 Bb4 D5, source score notes Bb4 D5 C5 Bb4 D5, the exact-source crop asset, local video sample \texttt{Njh8\_zq9\_DM-9465}, and local audio from 6.618--7.674 s inside that sample. The live practice-time surface reports 234 h 57 min uploaded archive video, 2 h 28 min checked video, 2 h 21 min measured active violin, 223 h 5 min estimated total active practice, and 232 h 28 min unchecked video. The estimate remains provisional until the full chronological active-practice scan covers every archive window.
...
M1 & Practice time & All 234 h 57 min of strict-ledger video has active-practice coverage and per-day totals. & Active-scan route deployed; live state now has 331 active intervals from 106 sample results, 2 h 30 min checked video, 2 h 21 min measured active practice, and 250 queued windows; full chronological scan still pending. \\
...
M10 & Regression lock & Tests prevent failed notation, wrong score boxes, and non-playing practice-time credit from returning. & Correction benchmark scaffold added; 3 user-reviewed wrong-score-note regressions are stored; truth-workbench write gates reject accepted audio-score note mismatches; visible-note-sequence verification is required for accepted score panels; full benchmark suite pending. \\
\bottomrule
\end{tabularx}
\end{table}
