...
Current accepted transcription & 1 score-linked seven-note Staff 4 measure-window is accepted: source \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5 C5}, detected audio \texttt{D\#5 D\#5 C5 D\#5 D\#5 D\#5 C5}, exact MIDI \texttt{75 75 72 75 75 75 72}. It is not a solved long phrase. Audio-checked D/A fragments remain useful note evidence, but they are not score-location matches or long-phrase transcription. \\
Current accepted score match & 1 actual-source score crop is accepted for the Staff 4 \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5 C5} window. The 2026-05-03 \texttt{Bb4 D5 C5 Bb4 D5} Scherzo-Tarantelle crop, the old A4/A5 single-note anchors, and the previous D--B-flat--G--D score extraction are all rejected fixtures. The current blocker is no longer the seven-note truth packet; it is \texttt{source\_extent\_exhausted}, meaning no verified Staff 4 source notes beyond the accepted seven-note lane are available yet. The 951 local score glyph candidates, 214 staff-position notehead hypotheses, 8 staff review packets, and source-verification targets are implementation queues, not accepted score evidence. \\
...
\paragraph{2026-05-18 Staff 4 full-phrase audit correction} The audit route now distinguishes a true failed adjacent note from a current exact-MIDI phrase that is merely waiting for source/truth acceptance. Before this correction, the seven-note target \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5 C5} could carry a full exact stored audio run while the packet generator still followed stale first-failure metadata and clipped only the first note for review. The \texttt{staff4\_phrase\_audit\_v3} packet ignores stale failed-note metadata whenever the current expansion already has matching target MIDI, audio MIDI, per-note audio agreement, and a source crop. The resulting packet is a full-phrase truth-review packet with local timing \texttt{20.225--23.280}, not a one-note blocker packet. A later 2026-05-18 truth promotion accepted that packet; the current blocker is now \texttt{source\_extent\_exhausted}, not \texttt{pending\_source\_lock}.
...
M2 & Score truth & Scherzo-Tarantelle has verified symbolic notes and score coordinates. & Advanced; nine opening notes and one Staff 4 seven-note accepted source/audio window are verified in MusicXML/source truth. Broader score coordinates and adjacent Staff 4 source notes remain pending. \\
...
M4 & Short sequence & Three-to-five played notes align to exact score notes and render with local audio/video. & Advanced past the short-sequence threshold; one May 3 seven-note audio phrase matches the verified Staff 4 source window by exact MIDI. Long phrases remain pending. \\
...
The 2026-05-18 Staff 4 seven-note truth promotion completes that review step. The full phrase \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5 C5} is accepted as source/audio truth against the detected audio spelling \texttt{D\#5 D\#5 C5 D\#5 D\#5 D\#5 C5}; both reduce to exact MIDI \texttt{75 75 72 75 75 75 72}. The accepted local window is \texttt{20.225--23.280} inside \texttt{Njh8\_zq9\_DM-8835}, and the accepted score crop is the same actual IMSLP Staff 4 source lane. The truth manifest now carries three live accepted Staff 4 source/audio phrases. The next state is intentionally blocked as \texttt{source\_extent\_exhausted}: Curtis has accepted the full verified Staff 4 source extent currently encoded, so the next implementation step is verifying more real MusicXML/source notes beyond the seven-note lane before asking the audio matcher for a longer phrase.
...
\begin{enumerate}
    \item keep the accepted seven-note Staff 4 lane as the only expansion base;
    \item extend the verified Staff 4 MusicXML/source map beyond the current \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5 C5} extent;
    \item create a new source crop for the adjacent verified notes from the actual IMSLP page, with centered boxes and exact source-range metadata;
    \item search stored and source-rescanned audio only after that new symbolic source extent exists;
...
Curtis now has the first verified seven-note measure-window before M1 is complete, but it still cannot claim a longer score match from guessed scanned crops, one-note anchors, reference-audio pitch traces, or source notes that have not been verified into MusicXML. The speed-up is to keep the accepted Staff 4 lane fixed, extend the source map from the real score, and add audio only when each added source note passes the same gates that a future long phrase will need.
