...
The tracker was refreshed through the 2026-05-15 12:29 PM EDT exact-note truth-gate pass. The system has a real media ledger, a deployed active-practice scan route, a working owner-sync path for captured media, a visible 100-point transcription-completion tracker with precise weighted progress, stored user-correction benchmarks, a persistent truth workbench for accepted/rejected/pending audio-score-transcription items, a local source-backed Wieniawski score PDF, a seven-note source-backed Wieniawski symbolic score opening phrase, a source-verification target queue, source-target score triage, an unaccepted score-glyph verification queue, an unaccepted score-note hypothesis queue, staff-level source review packets, and a conservative evidence gate. The existing trusted musical output is now short audio-checked note fragments only; the previously displayed source-backed five-note score/audio/video/transcription phrase lane is demoted because the visible notes in the actual source-score crop did not match the transcription. The current top source target is \texttt{D D D\# D D A\# G}; it has been checked against the current verified symbolic score map \texttt{D C A\# D C A\# D}, has only a one-note contiguous overlap, and is intentionally not accepted score evidence. The score-map workbench extracts 951 unaccepted source glyph candidates and 214 unaccepted staff-position notehead hypotheses across 8 local IMSLP staves, with 8 source review packets, so the next pass can verify score notes into MusicXML instead of searching the PDF manually. A score panel now requires explicit visible-note-sequence verification before it can count as a score-visual lock, actual-source score lock, measure match, or long phrase. Accepted score-truth items also require exact note-and-octave agreement; a same-pitch-class mismatch such as \texttt{A4} audio against \texttt{A5} score is rejected, and score notes without octave/register cannot become accepted score truth. One-note pitch anchors now require exact-note verification before they can render. This pass does not add accepted evidence by itself; it closes the gate that let wrong or ambiguous score crops look solved. The tracker is expected to remain around 45.626--45.628 percent exact progress after deployment if the live practice-time and score-queue state is otherwise unchanged. This is not solved long-phrase transcription yet; there are currently 0 accepted long phrases, 0 accepted measure matches, 0 exact score-aligned windows, 0 score-coordinate heat-map fragments, 0 score-visual locks, 0 actual-source score locks, 0 accepted truth items, and 0 verified source snippets. The previously displayed single-note score crops and the five-note 2026-05-03 crop remain rejected because the visible score-side notes did not match the displayed transcription.
...
Current accepted transcription & Short audio-checked D4/A4 fragments; not a full passage. \\
Current accepted score match & 0 accepted score matches after the 2026-05-03 five-note crop was demoted; detected notes \texttt{A\#4 D5 C5 A\#4 D5} remain a pending symbolic phrase candidate because the visible actual source crop did not show the same note sequence as the transcription; the current \texttt{D D D\# D D A\# G} source target is checked against the verified symbolic map and rejected for now with only 1/7 contiguous source overlap; 951 local score glyph candidates, 214 staff-position notehead hypotheses, 8 staff review packets, and 4 source-verification targets are queued for truth review but are not accepted score evidence. \\
Current archive coverage & 234 h 57 min uploaded practice video across 48 strict-ledger videos and 41 practice days; 2 h 30 min checked; 2 h 21 min measured active violin; full chronological coverage still pending. \\
Current estimate & 220 h 51 min total active practice estimated from checked windows; provisional and sample-biased until the full chronological scan covers the archive. \\
