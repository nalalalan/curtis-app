...
The tracker was refreshed through the 2026-05-15 08:37 PM EDT fast-transition candidate update. The system has a real media ledger, a deployed active-practice scan route, a working owner-sync path for captured media, a visible 100-point transcription-completion tracker with precise weighted progress, stored user-correction benchmarks, a persistent truth workbench for accepted/rejected/pending audio-score-transcription items, a local source-backed Wieniawski score PDF, a seven-note source-backed Wieniawski symbolic score opening phrase, a source-verification target queue, source-target score triage, an unaccepted score-glyph verification queue, an unaccepted score-note hypothesis queue, staff-level source review packets, a stricter evidence gate, and a hidden YIN fast-transition candidate trace for score matching. The 2026-05-03 Scherzo-Tarantelle five-note phrase \texttt{A\#4 D5 C5 A\#4 D5}, displayed as \texttt{Bb4 D5 C5 Bb4 D5}, now has one corrected actual-source crop that passes the strict source-score/audio/transcription gate. The old A4/A5 single-note anchors remain rejected, and a one-note pitch anchor still cannot render unless the score note and octave are explicitly verified. The current top longer source target \texttt{D D D\# D D A\# G} remains unaccepted because the verified symbolic score map contains only \texttt{D C A\# D C A\# D} and gives only a one-note contiguous overlap. The new transition trace is not displayed as notation and cannot promote uncertain octave-corrected notes; it exists to search fast note changes behind the strict exact-score gate. This is not solved long-phrase transcription yet; it creates one verified five-note lane, makes the daily-record API share the canonical active-practice coverage ledger, and keeps longer passage transcription, rhythm, repeated attempts, full-score mapping, and full archive practice-time coverage open.
...
Current accepted transcription & 1 score-linked five-note transcription phrase is accepted: \texttt{A\#4 D5 C5 A\#4 D5}, displayed as \texttt{Bb4 D5 C5 Bb4 D5}, paired with local audio/video. This is short-phrase progress only; no 10--30 s long phrase is solved. \\
Current accepted score match & 1 actual-source score crop is accepted for the 2026-05-03 Scherzo-Tarantelle phrase after visible source notes, transcription notes, octave/register, written spelling, score box centers, and paired audio agree under the strict gate. The current \texttt{D D D\# D D A\# G} longer source target is still rejected for now with only 1/7 contiguous source overlap; 951 local score glyph candidates, 214 staff-position notehead hypotheses, 8 staff review packets, and 2 source-verification targets are queued for truth review but are not accepted score evidence. \\
...
\paragraph{2026-05-15 06:34 PM EDT strict-source correction} The current pass replaces the visually rejected five-note crop with a corrected actual-source crop for the same 2026-05-03 Scherzo-Tarantelle phrase. The accepted detected sequence is \texttt{A\#4 D5 C5 A\#4 D5}; the displayed spelling is \texttt{Bb4 D5 C5 Bb4 D5}; the score-side sequence is the same octave-aware note sequence; and the score panel is a non-generated crop from the local Scherzo-Tarantelle solo part. Backend, frontend, and regression tests now require the same exact sequence before the group can count as an accepted measure-level match, exact score-aligned window, score-coordinate heat-map fragment, score-visual lock, actual-source score lock, verified source snippet, or accepted five-note phrase. This is the first accepted short source-score/audio phrase after the strict gate. It is still not solved long-phrase transcription: rhythm remains simplified, the phrase is about one second, and the full archive still needs many consecutive verified notes.

\paragraph{2026-05-15 08:37 PM EDT fast-transition candidate update} The next blocker was not another UI issue: the broader pYIN/onset stream in the 2026-05-03 Scherzo-Tarantelle window still collapses large parts of fast playing into repeated D/B-flat/G material, so a longer source-score phrase cannot be honestly promoted. Curtis now adds a hidden YIN frame-transition trace to every transcription run. This trace follows fast frame-level pitch changes and stores them as \texttt{scoreMatchCandidateNotes}; it is never rendered as accepted notation by itself. The daily matcher can search those hidden candidates against parsed source-score notes, but accepted score evidence still requires exact MIDI equality, exact written score spelling, visible source-score crop agreement, local audio/video, and no uncertain octave-corrected detected notes. Regression tests now prove that hidden transition candidates can feed the strict score matcher while uncertain octave-corrected candidates are rejected. A local check on the 5/3 \texttt{Njh8\_zq9\_DM-9465} clip produced 174 hidden fast-transition candidate notes with 8 pitch classes, but the only accepted source-score match remains the same five-note \texttt{Bb4 D5 C5 Bb4 D5} phrase because no longer exact source-score/audio run passed the gate. This is implementation progress toward long phrases, not a solved long phrase.
...
\paragraph{2026-05-15 09:20 PM EDT live targeted-reprocess verification} The deployed endpoint was run against \texttt{Njh8\_zq9\_DM-9465}. The live tracker then reported 67.375 percent exact weighted progress, 1380 hidden transition-trace candidates, 1 accepted long-phrase counter item, 1 accepted measure-match counter item, and 1 exact-source visible match group on 2026-05-03. Browser verification of the public site showed one visible accepted group: \texttt{Bb D C Bb D} in the score panel, local video/audio from \texttt{2:37:51--2:37:52}, and transcription \texttt{Bb4 D5 C5 Bb4 D5 / 1.1s}. This is the current floor for accepted evidence. It is still not the solved long-phrase system because the surrounding detected sequence still collapses much of the fast Scherzo playing into repeated B-flat/D events; the next real solve step is to extend verified source MusicXML and replace or calibrate the fast-passage pitch detector until a longer exact source-score/audio run survives the same gate.

\paragraph{2026-05-15 09:24 PM EDT post-deploy state check} A post-deploy API read showed that live background processing had advanced the implementation tracker to 68.125 percent exact weighted progress and 2476 hidden transition-trace candidates. The accepted visible evidence remained unchanged: one exact-source score/audio/video/transcription group for \texttt{Bb4 D5 C5 Bb4 D5} on 2026-05-03. The extra hidden candidates improve search coverage, but they do not count as solved transcription until a longer exact source-score run is verified.
...
M3 & One-note anchor & A displayed A from audio is matched to a verified score-side A, with no wrong-note crop. & Partially advanced; exact-note anchoring is proven inside the accepted \texttt{Bb4 D5 C5 Bb4 D5} phrase, while the old A-specific single-note anchors remain rejected. \\
M4 & Short sequence & Three-to-five played notes align to exact score notes and render with local audio/video. & Accepted for one five-note source-score/audio phrase; longer passages still require the full phrase gate. \\
