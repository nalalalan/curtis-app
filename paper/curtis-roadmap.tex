...
M1 & Practice time & All 205 h 24 min of strict-ledger video has active-practice coverage and per-day totals. & Pending. \\
...
M10 & Regression lock & Tests prevent failed notation, wrong score boxes, and non-playing practice-time credit from returning. & Pending. \\
\bottomrule
\end{tabularx}
\end{table}
...
After this PDF tracker, the next concrete implementation should be \textbf{full active-practice coverage plus benchmark/correction scaffolding}. This is the first useful code milestone because transcription cannot be trusted without knowing which source windows contain violin playing, and it cannot improve reliably without a gold correction loop.

The next coding pass should:
\begin{enumerate}
    \item add persistent scan coverage for every strict-ledger video;
    ...
    \item add a correction schema for note, score, match, piece, and active-interval errors;
    \item add tests for the known wrong-score-note failure mode;
