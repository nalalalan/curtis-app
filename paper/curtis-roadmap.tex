...
Current accepted score match & 1 actual-source score crop is accepted for the Staff 4 \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5 C5} window. The verified source map extends to the next \texttt{A4}, but that eight-note range is source-only because paired audio has not reproduced it. The 2026-05-03 \texttt{Bb4 D5 C5 Bb4 D5} Scherzo-Tarantelle crop, the old A4/A5 single-note anchors, and the previous D--B-flat--G--D score extraction are all rejected fixtures. The current blocker is exact audio coverage for \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5 C5 A4}, plus runtime access to the Staff 4 media file for audit artifacts. The 951 local score glyph candidates, 214 staff-position notehead hypotheses, 8 staff review packets, and source-verification targets are implementation queues, not accepted score evidence. \\
...
M2 & Score truth & Scherzo-Tarantelle has verified symbolic notes and score coordinates. & Advanced; nine opening notes, one Staff 4 seven-note accepted source/audio window, and one source-only eighth-note Staff 4 extension to \texttt{A4} are verified in MusicXML/source truth. Broader score coordinates and paired audio for the eighth note remain pending. \\
...
M10 & Regression lock & Tests prevent failed notation, wrong score boxes, and non-playing practice-time credit from returning. & Advanced; exact-MIDI manifest verifies four positive source/audio phrases plus one source-only Staff 4 extension, accepts three live Staff 4 source/audio phrases, blocks four false phrase maps, records the audited right-2 Staff 4 expansion as a rejected regression, moves \texttt{currentBest} to the eight-note right-1 source target, locks professional clef/notehead/accidental rendering, and keeps the eighth note hidden until paired audio exists. \\
\bottomrule
\end{tabularx}
\end{table}
