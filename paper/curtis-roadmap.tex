...
The tracker was refreshed through the 2026-05-14 05:23 AM EDT live deployment state. The system has a real media ledger, a deployed active-practice scan route, a working owner-sync path for captured media, a visible 100-point transcription-completion tracker with precise weighted progress, stored user-correction benchmarks, a local source-backed Wieniawski score PDF, a four-note source-backed Wieniawski symbolic score excerpt, and a conservative evidence gate, but long-phrase transcription is not solved. The existing trusted musical output is limited to short audio-checked note fragments plus one narrow source-backed score/audio measure match. The previously displayed single-note score crops remain rejected because the highlighted score-side note did not match the displayed note label.
...
Current accepted score match & 1 source-backed four-note Wieniawski measure match on 2026-05-03; detected pitch classes \texttt{D A\# G D} match the symbolic score \texttt{D A\# G D}; long-phrase count remains 0. \\
...
\paragraph{2026-05-14 05:23 AM EDT live symbolic-measure update} The deployed Curtis API now reports 43.642 percent exact progress, 1 accepted measure match, 1 exact score-aligned window, 0 accepted long phrases, 1 local score source, and 8 symbolic score notes counted across the two Wieniawski practice-day targets. The accepted 2026-05-03 Scherzo-Tarantelle group is a four-note source-backed symbolic score match: detected pitch classes \texttt{D A\# G D} match score pitch classes \texttt{D A\# G D}, and the score notes are \texttt{D5}, written \texttt{Bb4}, \texttt{G4}, and \texttt{D5} in the opening motif. This is the first deployed score/audio/video/transcription lane that passes the source-symbolic note gate. It is not a solved long phrase because the accepted run is four notes and one measure-level motif, not a longer passage with rhythm, repeats, heat-map coordinates, and performance observations.
\paragraph{2026-05-14 05:40 AM EDT source-target accounting update} The deployed Curtis API now reports 45.142 percent exact progress after the tracker began counting configured source score targets from the reference-training state instead of only local PDF files. The verified score-truth row now reports 3 score targets, 1 local score PDF, 8 symbolic score notes, and 1 exact score location. The accepted musical evidence is unchanged: one four-note source-backed Scherzo-Tarantelle measure match, one exact score-aligned window, and zero accepted long phrases. This makes the progress meter more honest and removes a hidden undercount, but it does not solve phrase transcription; the next implementation step is extending the accepted symbolic measure gate into longer matched note runs.
