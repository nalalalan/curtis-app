...
M10 & Regression lock & Tests prevent failed notation, wrong score boxes, and non-playing practice-time credit from returning. & Advanced; exact-MIDI manifest verifies three positive source phrases, accepts one live source/audio phrase, blocks three false phrase maps, exposes a 0/3 Staff 4 expansion gate that searches raw detected note series before stopping at the \texttt{Eb5} versus \texttt{D5} mismatch, and now generates a Staff 4 audit packet without accepting the failed expansion. \\
\bottomrule
\end{tabularx}
\end{table}
