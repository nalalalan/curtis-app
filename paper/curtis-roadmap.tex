...
\begin{enumerate}
    \item keep the accepted Staff 4 anchor fixed instead of starting a new random match;
    \item search nearby stored and newly extracted audio-note runs for the adjacent \texttt{Eb5} that would extend the source lane;
    ...
    \item record the first blocking note when an expansion fails so the next extraction attempt has a precise target;
