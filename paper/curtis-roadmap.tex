...
The tracker was refreshed through the 2026-05-18 Staff 4 full-phrase audit correction. The system has a real media ledger, a deployed active-practice scan route, a working owner-sync path for captured media, a visible 100-point transcription-completion tracker with precise weighted progress, stored user-correction benchmarks, a persistent truth workbench for accepted/rejected/pending audio-score-transcription items, a local source-backed Wieniawski score PDF, a nine-note source-backed Wieniawski symbolic score opening phrase, a source-verification target queue, source-target score triage, an unaccepted score-glyph verification queue, an unaccepted score-note hypothesis queue, staff-level source review packets, a stricter evidence gate, a hidden YIN fast-transition candidate trace for score matching, a Staff 4 phrase audit packet generator, a Staff 4 adjacent-mining pass, a Staff 4 source-audio rescanner with five overlapping source-window tiles, current-detector anchor reproduction, and a truth-manifest anchor fallback that keeps the accepted Staff 4 lane available even when the daily display has no promoted \texttt{matchGroups}. The Wieniawski opening source map is \texttt{A5 G5 F5 A5 G5 F5 A5 G\#5 F5}; the previous \texttt{D6 C6 Bb5 D6 C6 Bb5 D6 C\#6 B5} and lower-octave \texttt{D5 C5 Bb4 D5 C5 Bb4 D5} maps are rejected and covered by regression tests. The 2026-05-03 Scherzo-Tarantelle five-note phrase \texttt{A\#4 D5 C5 A\#4 D5}, displayed as \texttt{Bb4 D5 C5 Bb4 D5}, is explicitly rejected after Alan's review and cannot display as accepted score evidence. The old A4/A5 single-note anchors remain rejected, and a one-note pitch anchor still cannot render unless the score note and octave are explicitly verified. The accepted Staff 4 source/audio lane is now six notes: source spelling \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5}, detected audio spelling \texttt{D\#5 D\#5 C5 D\#5 D\#5 D\#5}, exact MIDI \texttt{75 75 72 75 75 75}, local timing \texttt{20.225--22.992}, and a centered actual-source crop from the local IMSLP solo-violin page. The right-2 seven-note lane now has exact audio MIDI \texttt{75 75 72 75 75 75 72} and full local timing \texttt{20.225--23.280}, but remains candidate/source material until the full source sequence, transcription, local audio/video, source crop range, and truth review agree. The transition trace is not displayed as notation and fails closed unless every matched fast-note candidate has a second detector agreement path, including spectral/onset agreement, sufficient confidence, and sufficient duration. Loose one- or two-note reference-audio coincidences no longer count as phrase progress, and candidate pitch-sequence groups no longer enter the daily \texttt{matchGroups} display bucket unless the exact source-score crop, visible note sequence, box centers, paired audio, transcription, score spelling, and accepted truth-evidence flags all pass. This is not solved long-phrase transcription yet; Curtis has one accepted six-note Staff 4 source/audio measure-window, but accepted long-phrase transcription remains blocked until a longer phrase survives audio, transcription, source-score, crop, and musician review.
...
Current accepted transcription & 1 score-linked six-note Staff 4 measure-window is accepted: source \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5}, detected audio \texttt{D\#5 D\#5 C5 D\#5 D\#5 D\#5}, exact MIDI \texttt{75 75 72 75 75 75}. It is not a solved long phrase. Audio-checked D/A fragments remain useful note evidence, but they are not score-location matches or long-phrase transcription. \\
Current accepted score match & 1 actual-source score crop is accepted for the Staff 4 \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5} window. The 2026-05-03 \texttt{Bb4 D5 C5 Bb4 D5} Scherzo-Tarantelle crop, the old A4/A5 single-note anchors, and the previous D--B-flat--G--D score extraction are all rejected fixtures. The right-2 \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5 C5} source lane now has a full exact-audio audit candidate, but it is still not accepted until its score/audio/crop/truth state is locked; 951 local score glyph candidates, 214 staff-position notehead hypotheses, 8 staff review packets, and source-verification targets are queued for truth review but are not accepted score evidence. \\
...
\paragraph{2026-05-18 Staff 4 full-phrase audit correction} The audit route now distinguishes a true failed adjacent note from a current exact-MIDI phrase that is merely waiting for source/truth acceptance. Before this correction, the seven-note target \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5 C5} could carry a full exact stored audio run while the packet generator still followed stale first-failure metadata and clipped only the first note for review. The \texttt{staff4\_phrase\_audit\_v3} packet ignores stale failed-note metadata whenever the current expansion already has matching target MIDI, audio MIDI, per-note audio agreement, and a source crop. The resulting packet is a full-phrase truth-review packet with local timing \texttt{20.225--23.280}, not a one-note blocker packet. It still cannot promote the lane by itself: without \texttt{truthEvidenceAccepted}, the correct status remains \texttt{pending\_source\_lock}.
...
M2 & Score truth & Scherzo-Tarantelle has verified symbolic notes and score coordinates. & Advanced; nine opening notes, one Staff 4 six-note accepted source/audio window, and one adjacent Staff 4 seven-note source-only extension are verified in MusicXML. Broader score coordinates remain pending. \\
...
M4 & Short sequence & Three-to-five played notes align to exact score notes and render with local audio/video. & Advanced past the short-sequence threshold; one May 3 six-note audio phrase matches the verified Staff 4 source window by exact MIDI. Long phrases remain pending. \\
...
The 2026-05-17 anchored-expansion harness implements that outward-extension step as a gate rather than a manual judgment. The solve tracker now starts from the accepted Staff 4 source/audio anchor and tests adjacent left and right source-note ranges against available audio-agreed note runs. Current result after the right-1 promotion: the six-note lane \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5} is accepted, and the seven-note right-side lane \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5 C5} remains the next audit target. This prevents Curtis from treating a new random phrase candidate as progress while the current accepted lane has not yet extended cleanly.
...
The 2026-05-17 10:46 PM EDT Staff 4 right-1 promotion converts the current exact audio candidate into accepted source/audio truth. The accepted phrase is source \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5}, detected audio \texttt{D\#5 D\#5 C5 D\#5 D\#5 D\#5}, exact MIDI \texttt{75 75 72 75 75 75}, local \texttt{20.225--22.992} inside \texttt{Njh8\_zq9\_DM-8835}, with the new actual-source crop \texttt{wieniawski-scherzo-tarantelle-staff4-eb-eb-c-eb-eb-eb-verified.png}. The truth manifest now has two live accepted Staff 4 source/audio phrases and four verified positive source phrases. The right-2 seven-note lane remains the next audit target; it cannot inherit acceptance from the six-note prefix.

The 2026-05-17 11:34 PM EDT anchor-advancement pass fixes the next source of circular work. The Staff 4 harness now prefers the longest accepted source/audio phrase that begins with the verified Staff 4 MIDI prefix, so the six-note accepted lane becomes the expansion base instead of the older five-note anchor. Expansion now moves rightward from that accepted lane; the previous left-side or shorter-prefix windows are no longer treated as the current task. The next source target is therefore exactly the seven-note lane \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5 C5}. If audio evidence for that full MIDI sequence is found, it can become \texttt{ready\_for\_truth\_review}; it still cannot display as accepted until the exact source crop, local audio/video, transcription glyphs, and truth gate all agree. The stale sixth-note failure wording was also retired because the sixth note is now accepted; the unresolved boundary is the seventh source note, \texttt{C5}.
...
The 2026-05-17 11:51 PM EDT live audit verification is the current endpoint state. The deployed API reports 69.113 percent implementation completion, reproduces the accepted six-note Staff 4 anchor under the v7 rescan, and finds an exact seven-note MIDI candidate: source \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5 C5}, detected audio \texttt{D\#5 D\#5 C5 D\#5 D\#5 D\#5 C5}, MIDI \texttt{75 75 72 75 75 75 72}. The Staff 4 audit packet \texttt{staff4-2026-05-03-Njh8\_zq9\_DM-8835-9-16} includes playable local audio and video artifacts plus the source crop, but its status remains \texttt{pending\_source\_lock} and truth decision remains \texttt{pending\_review}. This is the first seven-note exact-MIDI Staff 4 candidate; it is not accepted evidence until the source crop/range and musician truth review are locked.
...
\begin{enumerate}
    \item keep the accepted six-note Staff 4 lane as the only expansion base;
    \item generate or update the next Staff 4 seven-note audit packet for \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5 C5};
    \item compare the right-2 source sequence, detected sequence, source crop range, pitch trace, and local audio/video before any acceptance;
