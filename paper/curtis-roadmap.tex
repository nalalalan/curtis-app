...
The tracker was refreshed through the 2026-05-14 07:52 AM EDT live deployment state. The system has a real media ledger, a deployed active-practice scan route, a working owner-sync path for captured media, a visible 100-point transcription-completion tracker with precise weighted progress, stored user-correction benchmarks, a local source-backed Wieniawski score PDF, a six-note source-backed Wieniawski symbolic score opening phrase, one verified source-score crop, and a conservative evidence gate, but long-phrase transcription is not solved. The existing trusted musical output is limited to short audio-checked note fragments plus two narrow source-backed score/audio measure matches. The previously displayed single-note score crops remain rejected because the highlighted score-side note did not match the displayed note label.
...
M2 & Score truth & Scherzo-Tarantelle has verified symbolic notes and score coordinates. & Partially advanced; the local source PDF is present and a four-note symbolic opening motif is now parsed from source-backed MusicXML. More measures and score coordinates remain pending. \\
M3 & One-note anchor & A displayed A from audio is matched to a verified score-side A, with no wrong-note crop. & Pending. \\
M4 & Short sequence & Three-to-five played notes align to exact score notes and render with local audio/video. & Partially advanced; the matcher can now accept one source-backed four-note measure with local media, while long phrases still require the longer phrase gate. \\
