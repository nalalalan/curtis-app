...
\begin{enumerate}
    ...
    \item search adjacent stored and newly extracted detected note series against that same source lane;
...
\begin{enumerate}
    \item run the targeted production transcription refresh for the accepted Staff 4 sample and its neighboring May 3 windows so the raw detected-series pool is rebuilt with the current extractor;
    \item inspect whether any rebuilt raw run contains the adjacent \texttt{Eb5} after the accepted anchor;
