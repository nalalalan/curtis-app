...
The tracker was refreshed through the 2026-05-14 10:52 PM EDT visual-crop demotion pass. The system has a real media ledger, a deployed active-practice scan route, a working owner-sync path for captured media, a visible 100-point transcription-completion tracker with precise weighted progress, stored user-correction benchmarks, a local source-backed Wieniawski score PDF, a seven-note source-backed Wieniawski symbolic score opening phrase, a source-verification target queue, source-target score triage, an unaccepted score-glyph verification queue, an unaccepted score-note hypothesis queue, staff-level source review packets, and a conservative evidence gate. The existing trusted musical output is now short audio-checked note fragments only; the previously displayed source-backed five-note score/audio/video/transcription phrase lane is demoted because the visible notes in the actual source-score crop did not match the transcription. The current top source target is \texttt{D D D\# D D A\# G}; it has been checked against the current verified symbolic score map \texttt{D C A\# D C A\# D}, has only a one-note contiguous overlap, and is intentionally not accepted score evidence. The score-map workbench extracts 951 unaccepted source glyph candidates and 214 unaccepted staff-position notehead hypotheses across 8 local IMSLP staves, with 8 source review packets, so the next pass can verify score notes into MusicXML instead of searching the PDF manually. The tracker is expected to report 43.628 percent exact progress after deployment if the live practice-time and score-queue state is otherwise unchanged. This is not solved long-phrase transcription yet; there are currently 0 accepted long phrases, 0 accepted measure matches, 0 exact score-aligned windows, 0 score-coordinate heat-map fragments, 0 score-visual locks, 0 actual-source score locks, and 0 verified source snippets. The previously displayed single-note score crops and the five-note 2026-05-03 crop remain rejected because the visible score-side notes did not match the displayed transcription.
