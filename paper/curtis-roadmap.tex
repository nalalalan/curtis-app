...
\paragraph{2026-05-15 11:32 PM EDT live candidate-isolation verification} The deployed API reported 46.125 percent exact weighted implementation progress, 0 accepted long phrases, 0 accepted measures, 0 exact score-aligned windows, 0 actual-source score snippets, and 0 score-coordinate heat-map fragments. The 2026-05-03 daily record reported \texttt{source\_verification\_pending}, 0 visible \texttt{matchGroups}, 6 queued \texttt{candidateMatchGroups}, 0 score links, and 0 reference links. The top queued source target is \texttt{A D D F\# D}; it remains rejected as accepted score evidence because the current verified symbolic source map \texttt{D C A\# D C A\# D} gives only a one-note contiguous overlap.

\paragraph{2026-05-16 12:20 AM EDT source-octave correction} The verified symbolic score map itself had a register error. Rechecking the local IMSLP solo-violin page showed that the Wieniawski opening was encoded one octave too low. The MusicXML now stores \texttt{D6 C6 Bb5 D6 C6 Bb5 D6}; the old \texttt{D5 C5 Bb4 D5 C5 Bb4 D5} opening is a rejected regression case. The matcher still uses pitch-class overlap for candidate search, but accepted symbolic score matches now require exact MIDI equality against the corrected source map, per-note audio agreement, and the same actual-source crop and truth-evidence gates. This does not increase the accepted long-phrase count; it prevents a lower-octave phrase from being called a score match just because the note letters agree.
