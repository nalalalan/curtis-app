...
The tracker was refreshed through the 2026-05-15 05:00 PM EDT strict evidence-gate pass. The system has a real media ledger, a deployed active-practice scan route, a working owner-sync path for captured media, a visible 100-point transcription-completion tracker with precise weighted progress, stored user-correction benchmarks, a persistent truth workbench for accepted/rejected/pending audio-score-transcription items, a local source-backed Wieniawski score PDF, a seven-note source-backed Wieniawski symbolic score opening phrase, a source-verification target queue, source-target score triage, an unaccepted score-glyph verification queue, an unaccepted score-note hypothesis queue, staff-level source review packets, and a stricter evidence gate. The 2026-05-03 Scherzo-Tarantelle five-note source crop is no longer accepted: Alan's visual review showed that the visible noteheads and note boxes did not agree closely enough with the transcription and audio evidence. Curtis now withholds source score snippets unless score box centers, visible score notes, written spelling, exact note-plus-octave sequence, paired-audio transcription agreement, transcription-score agreement, and an accepted truth item all pass together. The old A4/A5 single-note anchors remain rejected, and a one-note pitch anchor still cannot render unless the score note and octave are explicitly verified. The current top longer source target \texttt{D D D\# D D A\# G} remains unaccepted because the verified symbolic score map contains only \texttt{D C A\# D C A\# D} and gives only a one-note contiguous overlap. This is not solved long-phrase transcription yet; it removes a false five-note score lane, makes the daily-record API share the canonical active-practice coverage ledger, and keeps longer passage transcription, rhythm, repeated attempts, full-score mapping, and full archive practice-time coverage open.
...
Current accepted transcription & 0 score-linked transcription phrases are currently accepted. The five-note Scherzo-Tarantelle run \texttt{A\#4 D5 C5 A\#4 D5} remains a pending audio-derived candidate, not accepted evidence. \\
Current accepted score match & 0 actual-source score crops are currently accepted. The previous \texttt{Bb4 D5 C5 Bb4 D5} crop at \texttt{mm. 2--4} is rejected until score box centers, visible source notes, transcription notes, and paired audio agree under the truth gate. The current \texttt{D D D\# D D A\# G} longer source target is still rejected for now with only 1/7 contiguous source overlap; 951 local score glyph candidates, 214 staff-position notehead hypotheses, 8 staff review packets, and 4 source-verification targets are queued for truth review but are not accepted score evidence. \\
