...
Current archive coverage & 213 h 23 min uploaded practice video across 46 strict-ledger videos and 39 practice days; 2 h 4 min 30 s checked; 1 h 57 min measured active violin; 211 h 19 min still unchecked. \\
Current estimate & 200 h 32 min total active practice estimated from checked windows; provisional and sample-biased until the full chronological scan covers the archive. \\
...
M1 & Practice time & All 213 h 23 min of strict-ledger video has active-practice coverage and per-day totals. & Active-scan route deployed; live state now has 242 active intervals from 88 sample results; queue-only owner sync checked \texttt{violin 1} through \texttt{*540-630}; full chronological scan still pending. \\
...
The 2026-05-13 06:01 PM EDT update consumed the next chronological queue window, \texttt{*540-630}. That window was violin-positive and added four active intervals after the active-practice scan. The run succeeded without a timeout at batch size one, and the next pending window is now \texttt{*630-720}. This keeps the current work focused on M1 coverage rather than score matching or long-phrase notation.
...
\begin{enumerate}
    \item keep running queue-only owner-media sync against the active-scan queue, starting with \texttt{violin 1} at \texttt{*630-720};
