...
The tracker was refreshed through the 2026-05-17 01:09 PM EDT Staff 4 anchor-reproduction update. The system has a real media ledger, a deployed active-practice scan route, a working owner-sync path for captured media, a visible 100-point transcription-completion tracker with precise weighted progress, stored user-correction benchmarks, a persistent truth workbench for accepted/rejected/pending audio-score-transcription items, a local source-backed Wieniawski score PDF, a nine-note source-backed Wieniawski symbolic score opening phrase, a source-verification target queue, source-target score triage, an unaccepted score-glyph verification queue, an unaccepted score-note hypothesis queue, staff-level source review packets, a stricter evidence gate, a hidden YIN fast-transition candidate trace for score matching, a Staff 4 phrase audit packet generator for the current expansion blocker, a Staff 4 adjacent-mining pass that searches stored May 3 detected audio-note windows for exact right-1 and right-2 MIDI sequences, a Staff 4 source-audio rescanner that now extracts up to five overlapping source-window tiles from the accepted anchor instead of one short padded clip, a current-detector anchor-reproduction check, and a truth-manifest anchor fallback that keeps that accepted Staff 4 lane available even when the daily display has no promoted \texttt{matchGroups}. The Wieniawski opening source map is now \texttt{A5 G5 F5 A5 G5 F5 A5 G\#5 F5}; the previous \texttt{D6 C6 Bb5 D6 C6 Bb5 D6 C\#6 B5} and lower-octave \texttt{D5 C5 Bb4 D5 C5 Bb4 D5} maps are rejected and covered by regression tests. The 2026-05-03 Scherzo-Tarantelle five-note phrase \texttt{A\#4 D5 C5 A\#4 D5}, displayed as \texttt{Bb4 D5 C5 Bb4 D5}, is explicitly rejected after Alan's review and cannot display as accepted score evidence. The old A4/A5 single-note anchors remain rejected, and a one-note pitch anchor still cannot render unless the score note and octave are explicitly verified. The current top longer source target \texttt{A D D F\# D} remains unaccepted because source-target checks now require exact detected MIDI values, per-note audio agreement, and a contiguous exact-MIDI run in the parsed MusicXML before a score phrase can display. The Staff 4 extension is blocked at \texttt{Eb5} versus \texttt{D5}; the first deployed tiled check produced four note runs and 286 primary-plus-candidate note events, then found zero exact adjacent Staff 4 candidates after 1547 stored/rescanned windows. The widened current-detector scan also did not reproduce the accepted five-note Staff 4 anchor, so the next implementation target is detector/segmentation calibration or a newly accepted current-detector anchor, not another blind source-window widening. If visible accepted match groups disappear, the source-audio rescanner and adjacent-mining pass still recover the accepted anchor from the truth manifest instead of reporting \texttt{no\_staff4\_anchor}, but the new reproduction status prevents that persistence from masquerading as fresh detector agreement. The transition trace is not displayed as notation and now fails closed unless every matched fast-note candidate has a second detector agreement path, including spectral/onset agreement, sufficient confidence, and sufficient duration. Loose one- or two-note reference-audio coincidences no longer count as phrase progress, and candidate pitch-sequence groups no longer enter the daily \texttt{matchGroups} display bucket unless the exact source-score crop, visible note sequence, box centers, paired audio, transcription, score spelling, and accepted truth-evidence flags all pass. This is not solved long-phrase transcription yet; Curtis has one accepted five-note Staff 4 source/audio measure-window, but accepted long-phrase transcription remains blocked until a longer phrase survives audio, transcription, source-score, crop, and musician review.
...
Current accepted transcription & 1 score-linked five-note Staff 4 measure-window is accepted: \texttt{Eb5 Eb5 C5 Eb5 Eb5} / MIDI \texttt{75 75 72 75 75}. It is not a solved long phrase. Audio-checked D/A fragments remain useful note evidence, but they are not score-location matches or long-phrase transcription. \\
Current accepted score match & 1 actual-source score crop is accepted for the Staff 4 \texttt{Eb5 Eb5 C5 Eb5 Eb5} window. The 2026-05-03 \texttt{Bb4 D5 C5 Bb4 D5} Scherzo-Tarantelle crop, the old A4/A5 single-note anchors, and the previous D--B-flat--G--D score extraction are all rejected fixtures. The current \texttt{A D D F\# D} longer source target is still rejected for now because it does not have an audio-agreed exact-MIDI run in the verified source MusicXML; 951 local score glyph candidates, 214 staff-position notehead hypotheses, 8 staff review packets, and 6 source-verification targets are queued for truth review but are not accepted score evidence. \\
...
\paragraph{2026-05-17 Staff 4 source-lane extension} The next source-verification step extends the current accepted Staff 4 lane outward instead of starting a new random score match. The verified MusicXML source now carries the adjacent \texttt{Eb5 C5} notes to the right of the accepted \texttt{Eb5 Eb5 C5 Eb5 Eb5} window, creating a seven-note source-only lane \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5 C5}. A new actual-score crop from the same local IMSLP page shows all seven noteheads with centered boxes. The visible accepted live match does not expand yet: the current May 3 audio phrase agrees for the first five MIDI values \texttt{75 75 72 75 75}, but the next detected audio note is \texttt{D5} / MIDI \texttt{74} while the extended source expects \texttt{Eb5} / MIDI \texttt{75}. Curtis therefore records the extension as verified source truth and as a source-positive regression fixture, but blocks it from becoming accepted live evidence. The regression tests now require this exact behavior so adjacent source growth cannot silently turn an audio mismatch into a displayed phrase match.

\paragraph{2026-05-17 Staff 4 phrase-audit packet update} Curtis now has a dedicated generator for the exact Staff 4 expansion blocker instead of retrying random score matches. The packet is keyed to the current source/audio lane, where the source target is \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5 C5} and the best stored audio run is \texttt{D\#5 D\#5 C5 D\#5 D\#5 D5 D\#5}. The generator extracts the bounded local audio clip, attempts a matching local video clip, preserves the stored detected notes, reruns pYIN, YIN, and spectral-peak pitch checks over the same audio, records onset times, writes a pitch-trace SVG, writes a spectrogram SVG, and stores the packet JSON under the runtime audit route. The packet can confirm the \texttt{Eb5} versus \texttt{D5} mismatch, expose detector disagreement, or queue manual audio review. It cannot promote the phrase to accepted evidence; its truth decision is explicitly \texttt{not\_accepted} until the source notes, audio notes, transcription, score crop, and review all agree.

\paragraph{2026-05-17 Staff 4 audit-rejection and next-window update} The right-2 Staff 4 audit result is now a rejected regression case in the long-phrase truth manifest. The rejected case binds the source target \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5 C5} to the observed paired audio run \texttt{D\#5 D\#5 C5 D\#5 D\#5 D5 D\#5}; because the sixth note is \texttt{D5} instead of source \texttt{Eb5}, that exact expansion cannot reappear as current accepted evidence. The expansion harness keeps the rejected window in the audit record, but no longer selects it as \texttt{currentBest}. The next adjacent Staff 4 candidate is now the right-1 window \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5}. It is tested separately and remains blocked at the same \texttt{Eb5} versus \texttt{D5} note until a different audio run agrees.
...
\textbf{Status: partially implemented.} The gold review loop now covers audio-phrase acceptance/rejection and score-phrase acceptance/rejection from queued detected clips. It is intentionally narrow: it can create accepted audio truth and exact score-ready truth, but it does not yet provide detailed onset/rhythm editing, score-box coordinate editing, batch keyboard review, waveform overlays, or full measure-level correction.
...
M2 & Score truth & Scherzo-Tarantelle has verified symbolic notes and score coordinates. & Advanced; nine opening notes, one Staff 4 five-note accepted source/audio window, and one adjacent Staff 4 source-only extension are verified in MusicXML. Broader score coordinates remain pending. \\
...
M4 & Short sequence & Three-to-five played notes align to exact score notes and render with local audio/video. & Partially advanced; one May 3 five-note audio phrase matches the verified Staff 4 source window by exact MIDI. Long phrases remain pending. \\
...
M10 & Regression lock & Tests prevent failed notation, wrong score boxes, and non-playing practice-time credit from returning. & Advanced; exact-MIDI manifest verifies three positive source phrases, accepts one live source/audio phrase, blocks four false phrase maps, records the audited right-2 Staff 4 expansion as a rejected regression, moves \texttt{currentBest} to the right-1 window, mines stored May 3 audio windows for exact right-1/right-2 MIDI candidates, and now rescans the original Staff 4 source audio before another expansion can be reviewed. \\
\bottomrule
\end{tabularx}
\end{table}
...
The 2026-05-17 anchored-expansion harness implements that outward-extension step as a gate rather than a manual judgment. The solve tracker now starts from the accepted Staff 4 source/audio anchor and tests the adjacent left and right source-note ranges against available audio-agreed note runs. Current result: 3 anchored expansions checked, 0 accepted. The strongest expansion target is the seven-note right-side lane \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5 C5}; it keeps the first five MIDI values exact, then blocks at \texttt{Eb5} versus \texttt{D5}. This prevents Curtis from treating a new random phrase candidate as progress while the current accepted lane has not yet extended cleanly.
...
The 2026-05-17 audit-rejection pass turns the audited right-2 failure into a permanent negative example instead of leaving it as the next task. The manifest now blocks four false phrase maps, including the Staff 4 source/audio expansion whose sixth note is \texttt{D5} against source \texttt{Eb5}. The expansion harness keeps that rejected case visible in the audit items but removes it from the current-best path. The next tested adjacent phrase is the right-1 Staff 4 window \texttt{Eb5 Eb5 C5 Eb5 Eb5 Eb5}; it also blocks at \texttt{Eb5} versus \texttt{D5}. This is useful because Curtis is no longer cycling on the same known-bad seven-note expansion.
...
The long-phrase track should now run in parallel with full-archive scanning. Waiting for all 234 h 57 min of archive coverage before extending one musical lane would be too slow. The faster strategy is a narrow accepted-measure lane:
\begin{enumerate}
    \item keep the accepted 2026-05-03 Staff 4 source/audio phrase as the anchor;
    \item keep the corresponding Scherzo-Tarantelle MusicXML measure-window and actual-score crop fixed as source truth;
    \item search adjacent stored and source-rescanned detected note series against that same source lane;
...
\begin{enumerate}
    \item inspect the live \texttt{guidedAdjacentTargets} output and use the first failed adjacent Staff 4 note as the next detector/segmentation calibration target;
    \item if every onset-sweep offset still fails, compare the exported first-failure object and best failed detector attempt against the Staff 4 audit packet pitch trace and decide whether the remaining error is pitch detection, onset placement, or the played note really being \texttt{D5};
    \item improve detector voting if the swept audio segment visibly/audibly contains the expected adjacent source note but pYIN/YIN/spectral disagreement blocks it;
    \item accept a longer compact grouped score--clip--audio--transcription pair only if the expanded exact MIDI sequence, paired audio, source crop, and truth gate all pass;
...
Curtis now has the first verified measure-window before M1 is complete, but it still cannot claim a longer score match from guessed scanned crops, one-note anchors, reference-audio pitch traces, or a mismatched adjacent audio note. The speed-up is to keep the accepted Staff 4 lane fixed and extend it only when each added source note passes the same gates that a future long phrase will need.
