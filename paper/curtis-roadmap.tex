...
Current archive coverage & 205 h 24 min uploaded practice video; 1 h 42 min checked for active violin; 29 min 18 s detected active violin; 203 h 42 min still unchecked. \\
Current estimate & 59 h total active practice estimated from checked windows; not final. \\
...
M1 & Practice time & All 205 h 24 min of strict-ledger video has active-practice coverage and per-day totals. & Coverage ledger/API added; full scan pending. \\
...
The remaining M1 work is the full low-cost media pass over every unmeasured strict-ledger video. The remaining M10 work is the larger benchmark suite for repeated notes, arpeggios, fast runs, score-free exercises, score-coordinate truth, and accepted notation/media completeness.

The next coding pass should:
\begin{enumerate}
    \item run a low-cost active-practice pass over all remaining unchecked videos;
    \item expose per-day uploaded duration, checked duration, detected active time, and estimate state;
    \item persist exact active intervals produced by that pass;
    \item expand the benchmark fixtures beyond the known wrong-score-note failure mode;
    \item update this paper with the new coverage numbers and any solved milestone.
\end{enumerate}
