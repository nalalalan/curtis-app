...
class TranscriptionCompletionTests(unittest.TestCase):
    ...
    def test_staff4_rejected_anchor_blocks_raw_detected_series_promotion(self):
        ...
        with patch(
            ...
            return_value={
                "status": "no_wide_exact_midi_in_source_window",
                "recordCount": 24,
                "continuousCandidateCount": 0,
            },
        ), patch(
    ...
    def test_source_crop_wide_attempt_search_finds_overlapping_exact_midi_window(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            source_path = Path(temp_dir) / "staff4-source.wav"
            write_tiny_wav(source_path, seconds=30)
            starts = []

            def fake_extract(_source, target, start, _end):
                starts.append(round(float(start), 3))
                write_tiny_wav(target, seconds=5)
                return True, ""

            def fake_transcribe(*_args):
                if starts and abs(starts[-1] - 12.0) < 0.01:
                    return {
                        "events": [
                            note("D#5", 0.06),
                            note("D#5", 0.20),
                        ...
                        "scoreMatchCandidateNotes": [],
                        "quality": {"segmentationSource": "test_source_crop_wide_attempt_search"},
                    }
                return {
                    "events": [note("D5", 0.06), note("A4", 0.20)],
                    "scoreMatchCandidateNotes": [],
                    "quality": {"segmentationSource": "test_source_crop_wide_attempt_search_empty"},
                }
            ...
            sample = {"id": "Njh8_zq9_DM-8835", "path": str(source_path), "window": "*8835-8865"}

            with patch("backend.app.scanner.run_ffmpeg_extract_audio", side_effect=fake_extract), patch(
                "backend.app.scanner.transcribe_audio_array",
                side_effect=fake_transcribe,
            ):
                search = source_crop_wide_attempt_search_for_target(target, [sample])

        self.assertEqual(search["status"], "continuous_exact_audio_candidate")
        self.assertIn(12.0, starts)
        self.assertEqual(search["targetMidiSequence"], [75, 75, 72, 75, 75, 75])
        self.assertGreaterEqual(search["recordCount"], 4)
        self.assertGreaterEqual(search["continuousCandidateCount"], 1)
        self.assertGreaterEqual(search["alternateAudioCandidateCount"], 1)
        self.assertTrue(search["bestCandidate"]["audioAgreed"])
