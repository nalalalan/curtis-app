...
class TranscriptionCompletionTests(unittest.TestCase):
    ...
    def test_staff4_phrase_expansion_gate_blocks_adjacent_audio_mismatch(self):
        ...
        self.assertEqual(harness["rejectedRegressionCount"], 1)
        rejected = [item for item in harness["items"] if item["status"] == "rejected_regression"]
        self.assertEqual(len(rejected), 1)
        self.assertEqual(rejected[0]["direction"], "right-2")
        self.assertEqual(rejected[0]["rejectedRegressionId"], "rejected-staff4-right2-audit-eb-vs-d")
        self.assertEqual(current["direction"], "right-1")
        self.assertEqual(current["status"], "blocked_audio_mismatch")
        self.assertEqual(current["targetSequence"], "Eb5 Eb5 C5 Eb5 Eb5 Eb5")
        self.assertEqual(current["bestAudioSequence"], "D#5 D#5 C5 D#5 D#5 D5")
    ...
    def test_staff4_phrase_expansion_searches_raw_detected_series(self):
        ...
        self.assertEqual(current["direction"], "right-1")
        self.assertEqual(current["targetSequence"], "Eb5 Eb5 C5 Eb5 Eb5 Eb5")
        self.assertEqual(current["bestAudioSequence"], "D#5 D#5 C5 D#5 D#5 D#5")
        self.assertEqual(current["bestExactCount"], 6)
        self.assertEqual(current["bestPrefixCount"], 6)
        self.assertEqual(current["audioRunSource"], "raw_detected_series")
        self.assertEqual(current["sourceImageUrl"], "/assets/score/wieniawski-scherzo-tarantelle-staff4-eb-eb-c-eb-eb-eb-verified.png")
        self.assertEqual(completion["phraseExpansionReadyForReviewCount"], 1)
        self.assertEqual(completion["phraseExpansionAcceptedCount"], 1)
        mining = completion["staff4AdjacentMining"]
        ...
        self.assertIn("Promote the accepted Staff 4 right-1 6-note expansion", completion["nextAction"])
    ...
    def test_staff4_source_audio_rescan_feeds_exact_midi_search(self):
        ...
        self.assertEqual(completion["phraseExpansionHarness"]["currentBest"]["targetSequence"], "Eb5 Eb5 C5 Eb5 Eb5 Eb5")
        self.assertEqual(completion["phraseExpansionHarness"]["currentBest"]["sourceImageUrl"], "/assets/score/wieniawski-scherzo-tarantelle-staff4-eb-eb-c-eb-eb-eb-verified.png")
        self.assertEqual(completion["staff4AdjacentMiningStatus"], "exact_audio_candidate")
